define("tui/searchBResults/view/SearchResultsComponent", [
  "dojo/_base/declare",
  "dojo/_base/connect",
  "dojo/_base/lang",
  "dojo/_base/xhr",
  ...
  "dojo/dom-class",
  "dojo/dom-geometry",
  "dojo/text!tui/searchBResults/view/templates/searchResultItemTmpl.html",
  ...
  "tui/searchB/nls/Searchi18nable"], function (declare, connect, lang, xhr, parser, aspect, topic, on, mouse, query,
  ...
  return declare("tui.searchBResults.view.SearchResultsComponent", [tui.widget._TuiBaseWidget, tui.widget.mixins.Templatable, tui.searchB.nls.Searchi18nable], {
   ...
   siteBrand:null,
    ...
    greatDealTestCode: null,

    greatdealtext1 : null,

    backToSearchFilterLoading : false,
    ...
    subscribeToChannels: function () {
      ...
      topic.subscribe("tui:channel=priceToggle", function (message) {
    	...
    	_.each(query('.btn-container .cta', resultsView.domNode), function(accomUrl){
      ...
      // update lazyload flag
      topic.subscribe("tui:channel=lazyloadfilter", function () {
    	  resultsView.backToSearchFilterLoading = true;
      });

      // remove updating class
      topic.subscribe("tui:channel=lazyload", function () {
    	if(!resultsView.backToSearchFilterLoading) {
        domClass.remove(query(".search-results")[0], 'updating');
    	}
    	resultsView.backToSearchFilterLoading = false;
        var holidayNode = {
    ...
    renderSearchResults: function (packages, isPagination, response) {
    	var resultsView = this,
    		html;
      ...
      var threePfInfo = response ? response.thirdPartyInfoMap : searchResponse.thirdPartyInfoMap;
      ...
      var holidays = _.map(lang.clone(packages), function (holiday, index) {
		...
		if(holiday.itinerary && holiday.itinerary.outbounds.length){
			holiday.itinerary.arrivalAirport = holiday.itinerary.outbounds[holiday.itinerary.outbounds.length - 1].arrivalAirport;
			if(holiday.itinerary.inbounds.length){
				holiday.dualCarrier = holiday.itinerary.outbounds[0].carrier.carrierCode === holiday.itinerary.inbounds[0].carrier.carrierCode ? false : true;
			}
		}
		if( ( holiday.inboundThirdParty || holiday.outboundThirdParty ) && !( holiday.inboundThirdParty && holiday.outboundThirdParty ) ) {
			holiday.mixedLeg = true;
		}

		if(threePfInfo && (holiday.inboundThirdParty || holiday.outboundThirdParty)){
	    	  resultsView.build3pfHolidayMap(holiday, threePfInfo);
	    }

		//stringfying the Itinerary details of each holiday and assigning to new varible to enable the flexibility of passing an object as an argument
        holiday.itineraryStr = JSON.stringify(holiday.itinerary).replace(/\'/g, "\\'");
      ...
      // todo: refactor
        ...
        html = resultsView.renderTmpl(null, lang.mixin(resultsView.analyticsData, {
          ...
          isThFcSite : resultsView.siteName === "thomson" || resultsView.siteName === "firstchoice",
          currency : resultsView.currency,
          displayDiscount: JSON.parse(resultsView.displayDiscount),
          greatdealtext: resultsView.greatdealtext,
          greatDealcolor: resultsView.greatDealcolor,
          greatDealTestCode: resultsView.greatDealTestCode,
          greatdealtext1: resultsView.greatdealtext1,
          threePfVariant: resultsView.threePfVariant,
          thirdPartyInfoMap : threePfInfo
        }));
    ...
    build3pfHolidayMap : function (holiday, thirdPartyInfoMap){
    	var carrierCode = "",
    		resultsView = this;
    	// embedd 3pf data into individual holiday level
			if(holiday.itinerary && holiday.itinerary.inbounds.length){
				_.each(holiday.itinerary.inbounds, function(inboundCarrier){
					carrierCode = inboundCarrier.carrier.carrierCode;
					if(thirdPartyInfoMap[carrierCode]){
						inboundCarrier.carrier.thirdPartyInfo = resultsView.buildFlightDetailsMap(carrierCode,thirdPartyInfoMap[carrierCode]);
					}
				});
			}
			if(holiday.itinerary && holiday.itinerary.outbounds.length){
				_.each(holiday.itinerary.outbounds, function(outboundCarrier){
					carrierCode = outboundCarrier.carrier.carrierCode;
					if(thirdPartyInfoMap[carrierCode]){
						outboundCarrier.carrier.thirdPartyInfo = resultsView.buildFlightDetailsMap(carrierCode,thirdPartyInfoMap[carrierCode]);
					}
				});
			}
    },

    buildFlightDetailsMap : function(carrierCode, thirdPartyInfoMap){
         return {
				"logoUrl" : thirdPartyInfoMap[carrierCode + "_ThirdPartyFlightImage"],
				"title" : thirdPartyInfoMap[carrierCode + "_ThirdPartyFlightText"] ? thirdPartyInfoMap[carrierCode + "_ThirdPartyFlightText"] : "",
				"aimText" : thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText1"] ? thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText1"] : "",
				"otherCarrierText" : thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText2"] ? thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText2"] : "",
				"assuranceText"  : thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText3"] ? thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText3"] : ""  ,
				"changeCarrierText" : thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText4"] ? thirdPartyInfoMap[carrierCode + "_ThirdPartyFlight_SearchIntroText4"] : ""
		}
    },
    ...
    delegateEvents: function () {
      ...
      on(resultsView.domNode, on.selector(".url", "click"), function (event) {
        ...
        topic.publish('tui/searchBResults/Mediator/saveState', []);
      });
      ...
      on(resultsView.domNode, on.selector(".shortlist", mouse.leave), function (event) {
        var itemNode = query(event.target).parents(".search-result-item")[0];
        if (resultsView.isShortlisted(itemNode)) {
        	if(resultsView.siteName=="firstchoice" || resultsView.siteName=="thomson"){
    ...
    on(resultsView.domNode, on.selector(".toggle p", "click"), function (event) {
      var targetElement = this,
      toggleParent = targetElement.parentElement.parentElement;
      hideLiElements = dojo.query("li.board-name", toggleParent);
          var iconElement = dojo.query("i", targetElement)[0];
          if(domClass.contains(iconElement, "arrow-down"))
            targetElement.innerHTML = "Hide board options <i class='arrow-up'></i>";
          else
            targetElement.innerHTML = "More board options <i class='arrow-down'></i>";
          _.each(hideLiElements, function (liItem) {
              if(domClass.contains(liItem, "hide")) domClass.remove(liItem,"hide");
              else domClass.add(liItem, "hide");
          });
      });
    ...
    toggleShortlistedStatus: function (add, holidayItem) {
     ...
     if(resultsView.siteName=="firstchoice" || resultsView.siteName=="thomson"){
      ...
      if( text == "saved" &&  query(".list-viewport ul.plist li.search-result-item").indexOf(holidayItem) != -1 ){
	      ...
	      var continueBtn = query(".btn-container .url", holidayItem)[0];
    ...
    refresh: function (field, oldValue, newValue, response, isCached) {
      ...
      switch (field) {
        case 'paginate' :
          ...
          resultsView.renderSearchResults(responseHolidays, true, response);
          break;
        default :
          ...
          resultsView.renderSearchResults(responseHolidays, false, response);
          break;
      }
    },
