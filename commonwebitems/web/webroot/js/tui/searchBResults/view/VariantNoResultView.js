define("tui/searchBResults/view/VariantNoResultView", [
  "dojo",
  "dojo/on",
  "dojo/query",
  "tui/utils/AjaxQueue",
  "dojo/text!tui/searchBResults/view/templates/noResultsAltVariantSearchTmpl.html",
  "dojo/io-query",
  "tui/searchBResults/view/NoResultView",
  "tui/widget/_TuiBaseWidget"], function (dojo, on, query, ajaxQueue, tmpl, ioQuery, tagMappingTable) {

  var widenRequestTypes = ["WHEREFROM", "WHERETO", "WHEN", "DURATION"];//,

  dojo.declare("tui.searchBResults.view.VariantNoResultView", [tui.searchBResults.view.NoResultView], {
    ...
    postCreate: function () {
      ...
      on(noResultView.domNode, on.selector(".notice a", "click"), function (event) {
    	  if(noResultView.abTestCode == "phoenixB"){
    ...
    generateUrl: function (params) {
      var noResultView = this, queryParams, airports = [], units = [],
          paramsToSend = ["when", "until", "flexibility", "flexibleDays", "noOfAdults", "noOfSeniors",
            "noOfChildren", "duration", "first","flexibleMonths"];
      ...
      queryParams = dojo.mixin(queryParams, {
        "from[]": airports.join("|"),
        "to[]": units.join("|"),
        "childrenAge": params.childrenAges.join(','),
  ...
  return tui.searchBResults.view.VariantNoResultView;
});