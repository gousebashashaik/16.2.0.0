define("tui/searchBPanel/view/SeniorsSelectOption", [
	"dojo",
	"tui/searchBPanel/view/PartyCompSelectOption"], function (dojo) {


	dojo.declare("tui.searchBPanel.view.SeniorsSelectOption", [tui.searchBPanel.view.PartyCompSelectOption], {
		...
		updatePartyCompError: function(name, oldError, newError){
			...
			// publish error for listeners
			if (_.size(newError) === 1) {
				dojo.publish("tui/searchBPanel/view/PartyCompositionView/Errors", [seniorsSelectOption.widgetController, true]);
			}
			if (_.size(newError) === 0) {
				dojo.publish("tui/searchBPanel/view/PartyCompositionView/Errors", [seniorsSelectOption.widgetController, false]);
			}
	...
	return tui.searchBPanel.view.SeniorsSelectOption;
});
