define("tui/searchBPanel/view/DestinationMultiFieldList", ["dojo",
  "dojo/cookie",
  "dojo/on",
...
], function (dojo, cookie, on, strings, searchConfig, ErrorPopup, DestinationModel, TuiConfig) {

  dojo.declare("tui.searchBPanel.view.DestinationMultiFieldList", [tui.searchBPanel.view.SearchMultiFieldList,
	tui.searchBPanel.view.SearchErrorMessaging], {
	...
	addOnChangeEventListener: function() {
      ...
      resultSet.observe(function (destinationModel, remove, add) {
        ...
        if(dojoConfig.site == "thomson"){
			 if(destinationModel.name == "Sensimar"){
				destinationModel.name = "COUPLES/SENSIMAR";
			}
		}
		
		if(destinationMultiFieldList.tuiConfig[dojoConfig.site] && dojoConfig.dualBrandSwitch) {
	    	  labelValue = destinationMultiFieldList.tuiConfig[dojoConfig.site].dualBrandConfig.optionLabel[destinationModel.name.toLowerCase().replace(/\s/g,"")];
	    	  if(labelValue){
	    		  destinationModel.name = labelValue;
	    	  }
	      }
        ...
        if(!destinationModel.multiSelect){
        	...
        	if(action == "removeTextbox"){
        		...
        		if(dojo.query("ul.howLongDD li:first-child")[0]){
        		...
        		if(dojo.query("ul.howLongDD li.lapLandDDLi")[0]){
       ...
       if(destinationModel.fewDaysFlag || destinationModel.id ==="ISL") {
    	   ...
    	   if(action === "removeTextbox"){
        	...
        	dojo.query("ul.howLongDD li.aFewDayTripLi").addClass("active");
        	if(dojo.query("ul.howLongDD li.aFewDayTripLi")[0]){
        		on.emit(dojo.query("ul.howLongDD li.aFewDayTripLi")[0], "mouseover", {
					bubbles: true,
					cancelable: true
				});
        		on.emit(dojo.query("ul.howLongDD li.aFewDayTripLi")[0], "click", {
					bubbles: true,
					cancelable: true
				});
  ...
  /*     on(destinationMultiFieldList.infoPopup.popupDomNode, on.selector('.destination-guide', "click"), function (event) {
      ...
      on(destinationMultiFieldList.infoPopup.popupDomNode, on.selector('.datepicker', "click"), function (event) {
        ...
        // open datepicker
        dojo.publish("tui.searchBPanel.view.SearchDatePicker.onCalendarFocus", destinationMultiFieldList.widgetController.searchApi);
      });
    },
  ...
  updatePlaceholder: function () {
      ...
      destinationMultiFieldList.placeholder.innerHTML = text;
    }

  });

  return tui.searchBPanel.view.DestinationMultiFieldList;
});
