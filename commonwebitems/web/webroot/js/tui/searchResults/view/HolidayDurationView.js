define("tui/searchResults/view/HolidayDurationView", ["dojo",
  "dojo/dom-attr",
  "dojo/on",
  "dojo/_base/lang",
  "dojo/keys",
  "dojo/text!tui/searchResults/view/templates/holidayDurationTmpl.html",
  "dojo/topic",
  "dojo/cookie",
  ...
  dojo.declare("tui.searchResults.view.HolidayDurationView", [tui.widget._TuiBaseWidget, tui.widget.mixins.Templatable, tui.search.nls.Searchi18nable], {
    ...
    handleNoResults: function (name, oldDuration, newDuration) {
      ...
      if (name === 'duration') {
        ...
        if (holidayDurationView.isCustomDuration(newDuration)) {
          ...
          topic.publish("tui.searchResults.view.SearchResultsComponent.renderNoResultsPopup");
        }
    ...
    postCreate: function () {
      ...
      holidayDurationView.moreChoicesMenu = dojo.query(".custom-dates", holidayDurationView.domNode);
      ...
      holidayDurationView.tagElements(dojo.query(".duration-standard", holidayDurationView.domNode), function (DOMElement) {
        ...
        return aElement && aElement.innerHTML;
      });
      if( dojo.query(".search-results .single-accommodation-details .product").length ){
    	  holidayDurationView.tagElements(dojo.query(".custom-dates", holidayDurationView.domNode), "SASseeDurations");
      }else{
      holidayDurationView.tagElements(dojo.query(".custom-dates", holidayDurationView.domNode), "More Durations");
      }
    ...
    attachDurationViewEventListeners: function () {
      ...
      on(holidayDurationView.domNode, on.selector(".custom-dates", "click, keyup"), function (event) {
        dojo.stopEvent(event);
        if (event.keyCode && event.keyCode !== keys.ENTER) return;
    ...
    addCustomTab: function (value) {
      ...
      dojo.place(html, dojo.query(".custom-dates", holidayDurationView.domNode)[0], "before");
    ...
    updateCustomTab: function (value) {
      ...
      a.innerHTML = value + ' ' + (parseInt(value, 10) > 1 ? holidayDurationView.searchMessaging.nights : holidayDurationView.searchMessaging.night);
    },
  ...
  return tui.searchResults.view.HolidayDurationView;
});