define('tui/searchResults/view/DateAvailabilitySlider', [
  'dojo',
  'dojo/date/locale',
  'dojo/on',
    'dojo/query',
...
], function(dojo, locale, on, query) {

  dojo.declare('tui.searchResults.view.DateAvailabilitySlider', [tui.widget.form.sliders.Slider], {
	...
	animateRightInterval: null,
        ...
        refresh: function (field, oldSelection, newSelection, response, isCached, sliderPPChanged, sliderTotalChanged, dateSliderChanged) {
      ...
      var listOfDates = dojo.query('span.date-slide', li)[0].innerHTML;
      _.each(listOfDates, function (i) {
                if (listOfDates == parseInt(date[0])) {
        	  dojo.addClass(li, 'curr-date-highlight');
        	  }
        });
      ...
      var availableDatesList = dojo.query('.slidey-calendar li').length;
        	if(availableDatesList > 8 & availableDatesList <=16){
        dojo.query('.slidey-calendar li div span').addClass('date-slide-7');
        	}
            if(availableDatesList > 16){
                if(availableDatesList > 24){
          dojo.query('.slidey-calendar ul li').addClass('marker-25');
                }else{
          dojo.query('.slidey-calendar ul li').addClass('marker-17');
                }
        dojo.setStyle(dojo.byId('dateAvailabilitySlider'), 'height', '64px');
        dojo.setStyle(dojo.query('.slidey-calendar ul')[0], 'top', '40px');
            }
      ...
      var liLeft = dojo.style(dojo.query('.day:not(.day.outside):first', wrap).parent()[0], 'left');
      var liWidth = dojo.style(dojo.query('.day:not(.day.outside):first', wrap).parent()[0], 'width');
        	//var liHeight = dojo.style(dojo.query(".day:not(.day.outside):first", wrap).parent()[0], "height");
      var liCount = dojo.query('.day:not(.day.outside)', wrap).length;
        	var dateGap = 0; 
      if (dojo.isIE === 8) {
      ...
      if (!dojo.query('.track .runner-indicator', wrap).length) {
        ...
        if (dojo.query('.slidey-calendar li').length > 16) {
          if (dojo.query('.slidey-calendar li').length > 24) {
            ...
            if (!dateAvailabilitySlider.toolTipWidgets) {
                dojo.forEach(lis, function(li, listIndex) {
                    ...
                    dojo.attr(toolTipSpan[0], {
                        'data-dojo-type': 'tui.searchResults.view.Tooltips',
                ...
                //var toolTipMsg = "floatWhere:'position-top-center', text:'" + msg + "'";
        //dojo.attr(toolTIpSpan[0], {'data-dojo-type': 'tui.searchResults.view.Tooltips', 'data-dojo-props': toolTipMsg});
             });
            }
        },
    ...
    return tui.searchResults.view.DateAvailabilitySlider;
});
