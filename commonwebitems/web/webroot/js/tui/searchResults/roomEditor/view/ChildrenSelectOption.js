define("tui/searchResults/roomEditor/view/ChildrenSelectOption", [
  "dojo",
  "dojo/on",
  ...
  dojo.declare("tui.searchResults.roomEditor.view.ChildrenSelectOption", [tui.searchResults.roomEditor.view.PartyCompSelectOption, tui.widget.mixins.Templatable], {
    ...
    postCreate: function () {
      ...
      childrenSelectOption.subscribe("tui/searchResults/roomEditor/view/RoomsEditor/Errors/Party", function (action) {
        action = action ? "addClass" : "removeClass";
        dojo[action](childrenSelectOption.domNode, "error");
      });
      if( dojoConfig.site == "cruise"){
    	  var tagTxt = "c" + childrenSelectOption.roomModel.id.replace("room-", "") + "Children";
    	  childrenSelectOption.tagElement(childrenSelectOption.domNode, tagTxt);
      }else{
    ...
    addChildAgeSelectOptions: function (size) {
      ...
      _.forEach(dojo.query(".child-age", childrenSelectOption.unitNode), function (item, i) {
        ...
        select.subscribe("tui/searchResults/roomEditor/view/RoomsEditor/Errors/InfantLimit", function (action) {
          action = action ? "addClass" : "removeClass";
          dojo[action](select.domNode, "error")
        });
  ...
  return tui.searchResults.roomEditor.view.ChildrenSelectOption;
});