define("tui/widget/media/MediaPopup", [
  'dojo',
  'dojo/on',
  'dojo/dom-attr',
  'dojo/_base/fx',
  ...
  dojo.declare("tui.widget.media.MediaPopup", [tui.widget.popup.Popup, tui.widget._TuiBaseWidget], {
    ...
    getcaption: function (node) {
        ...
        var captionEl = dojo.query('.large-carousel p', mediaPopup.popupDomNode)[0];
    ...
    createNextSlide: function () {
      ...
      if (mediaPopup.currentSlide === mediaPopup.slidesNo - 1) {
        ...
        test=query("html")[0];
        if(mediaPopup.checkForTouchClass()){
        	mediaPopup.end = mediaPopup.slideAmountforIpad * (mediaPopup.currentSlide + 1);
        }
        else {
    ...
    checkForTouchClass: function(){

    	return dojo.hasClass(document.body, "touch") || dojo.hasClass(test, "touch")
    ...
    createPrevSlide: function () {
      ...
      if (mediaPopup.currentSlide === 0) {
        ...
        mediaPopup.start = mediaPopup.end;
        if(dojo.hasClass(document.body, "touch")){
        	mediaPopup.end = mediaPopup.slideAmountforIpad * (mediaPopup.currentSlide - 1);
        }
        else {
        mediaPopup.end = mediaPopup.slideAmount * (mediaPopup.currentSlide - 1);
    ...
    onClose: function () {
      ...
      //dojo.window.scrollIntoView(mediaPopup.domNode); BZ 139923 fixed
    }
