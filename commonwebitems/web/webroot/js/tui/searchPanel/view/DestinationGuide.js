define('tui/searchPanel/view/DestinationGuide', [
  'dojo',
  'dojo/_base/lang',
  'dojo/query',
  'dojo/on',
  'dojo/dom-attr',
  'dojo/dom-class',
  'dojo/text!tui/searchPanel/view/templates/DestinationGuideTmpl.html',
  ...
  'dojo/text!tui/searchPanel/view/templates/DestinationGuideSelectOptionTmpl.html',
...
], function (dojo, lang, query, on, domAttr, domClass, destGuideTmpl, recDestTmpl, countryDestTmpl, inspireTmpl, selOptionTmpl, DestinationModel, DestinationGuideStore, DestinationGuideSelectOption, ListFormatter, TuiConfig) {

  dojo.declare('tui.searchPanel.view.DestinationGuide', [tui.searchPanel.view.SearchGuide], {
	...
	dualBrandClicks: 0, //dualBranding
    ...
    postCreate: function () {
      ...
      var brandURL= dojoConfig.site == "firstchoice"?true:false;
      if(brandURL){
    	  destinationGuide.url = dojoConfig.paths.webRoot + '/destinations';
      }
      else {
    	  destinationGuide.url = dojoConfig.paths.webRoot + '/holiday-destinations.html';
      }
      ...
      if (guideData.value === "") {
        ...
        dojo.when(destinationGuide.destinationGuideStore.requestData(null, destinationGuide.searchPanelModel.generateQueryObject()), function (response) {
		  if(destinationGuide.tuiConfig[dojoConfig.site] && dojoConfig.dualBrandSwitch) {
		      _.each(response.suggestions[1].children, function (item, index) {
		    	  labelValue = destinationGuide.tuiConfig[dojoConfig.site].dualBrandConfig.optionLabel[item.name.toLowerCase().replace(/\s/g,"")];
		    	  if(labelValue){
		    		  response.suggestions[1].children[index].name = labelValue;
		    	  }
		      });
	      }
		  if(dojoConfig.site == "thomson"){
		  _.forEach(response.suggestions[1].children, function (item, index) {
				if (item.name == "Sensimar"){
					response.suggestions[1].children[index].name = "COUPLES/SENSIMAR";
				}
			});
		   }			
    ...
    attachOpenEvent: function () {
      ...
      on(destinationGuide.domNode, "click", function (event) {
        ...
        if (destinationGuide.expandableDom === null || !destinationGuide.isShowing(destinationGuide.expandableDom)) {
          dojo.publish("tui.searchPanel.view.AirportGuide.closeExpandable");
          setTimeout(function () {
    ...
    onAfterTmplRender: function () {
      ...
      on(destinationGuide.expandableDom, "label:click", function (event) {
		...
		//dualbranding
		if(dojo.query("#collection-SMR").parent().attr("class")[0] == dojo.query("#collection-COU").parent().attr("class")[0]){
			destinationGuide.dualBrandClicks = 0;
		}
        var checkbox, label;
        ...
        dojo.publish("tui.searchPanel.view.DestinationMultiFieldList.pulse", [destinationGuide.widgetController]);
		...
		//dualbranding
		if(dojoConfig.site == "thomson"){
			if(checkbox.id == "collection-SMR"){ 			
				destinationGuide.dualBrandClicks++;
				if(destinationGuide.dualBrandClicks < 2){
				dojo.query("#collection-COU")[0].click(function(){						
					return;
				});
				}
			}
			if(checkbox.id == "collection-COU"){ 			    
				destinationGuide.dualBrandClicks++;
				if(destinationGuide.dualBrandClicks < 2){
				dojo.query("#collection-SMR")[0].click(function(){						
					return;
				});
				}
			}
    ...
    openExpandable: function () {
      ...
      dojo.publish("tui.searchPanel.view.DestinationMultiFieldList.highlight", [true, destinationGuide.widgetController]);
      ...
      dojo.when(destinationGuide.destinationGuideStore.requestData(null, destinationGuide.searchPanelModel.generateQueryObject()), function (response) {
        ...
        //Commented out as part of performance fix, US20050->TA49713
      //  if(!destinationGuide.firstRender) dojo.publish("tui.searchPanel.view.DestinationGuideSelectOption.setSelectedValue", [""]);
            ...
            if (destinationModel) {
              ...
              if (children) {
                ...
                _.each(children, function (childrenId) {
                  ...
                  if (!childrenId || childrenId === "" || _.indexOf(clickedCheckboxChildren, childrenId) >= 0 ||
    ...
    updateGuideTitle: function () {
      ...
      airportModel = _.map(airportModel, function (obj) {
        return obj.name;
      });
  ...
  return tui.searchPanel.view.DestinationGuide;
});