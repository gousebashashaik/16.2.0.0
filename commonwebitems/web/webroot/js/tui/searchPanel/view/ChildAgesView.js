define("tui/searchPanel/view/ChildAgesView", [
  "dojo",
  "dojo/query",
  "tui/searchPanel/view/SearchErrorMessaging",
  "tui/widget/_TuiBaseWidget"], function (dojo, query) {

  dojo.declare("tui.searchPanel.view.ChildAgesView", [tui.widget._TuiBaseWidget, tui.searchPanel.view.SearchErrorMessaging], {
    ...
    postCreate: function () {
      ...
      childAgesView.watcher = childAgesView.searchPanelModel.searchErrorMessages.watch("partyChildAges", function (name, oldError, newError) {
        ...
        if (_.size(newError) === 1) {
          dojo.publish("tui/searchPanel/view/ChildAgesView/Errors", [childAgesView.widgetController, true]);
        }
        if (_.size(newError) === 0) {
          dojo.publish("tui/searchPanel/view/ChildAgesView/Errors", [childAgesView.widgetController, false]);
        }
      });
    }
  });

  return tui.searchPanel.view.ChildAgesView;
});