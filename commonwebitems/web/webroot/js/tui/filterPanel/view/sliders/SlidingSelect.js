define('tui/filterPanel/view/sliders/SlidingSelect', [
  'dojo',
  'dojo/query',
  'dojo/dom-class',
  'dojo/dom-style',
  'dojo/on',
  'dojo/dom-attr',
  'tui/widget/_TuiBaseWidget'], function(dojo, query, domClass, domStyle, on, domAttr) {

  dojo.declare('tui.filterPanel.view.sliders.SlidingSelect', [tui.widget._TuiBaseWidget], {
    ...
    updateSlider: function(i) {
      ...
      if(slider.trackType === "maxRange") {
        ...
        width = (i + 1) * slider.stepWidth + slider.steps;
      ...
      if(slider.trackType === "minRange") {
        if(slider.offset === 0) {
          ...
          width = i === 0 ? ((slider.steps + 1) * slider.stepWidth - 1) : ((slider.steps - (i + 1)) + 1) * slider.stepWidth - 1;
        }
        domStyle.set(slider.slider, 'width', width + 'px');
        _.each(slider.trackLists(), function(pill, index) {
          if(!domClass.contains(pill, 'disabled')) {
    ...
    attachTrackEvents: function() {
      var slider = this, pill = query('ul > li', slider.domNode)[0];

      _.each(slider.trackLists(), function(pill, i) {
        on(pill, 'click', function(e) {
          ...
          if(domAttr.get(pill, 'data-value') != null)
            slider.onSelect(domAttr.get(pill, 'data-value'), i);
          else
            slider.onSelect(domAttr.get(pill, 'data-value-total'), i);
