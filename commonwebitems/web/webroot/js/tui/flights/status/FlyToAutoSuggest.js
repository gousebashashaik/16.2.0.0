define("tui/flights/status/FlyToAutoSuggest", [
	"dojo",
	"dojo/text!tui/widget/form/flights/templates/statusAutoCompTmp.html",
	"dojo/on",
	"tui/flights/status/FlyToAutoComplete",
    ...
    dojo.declare("tui.flights.status.FlyToAutoSuggest", [tui.widget._TuiBaseWidget, dijit._TemplatedMixin, tui.widget.mixins.Scrollable, tui.utils.CancelBlurEventListener], {

        // ---------------------------------------------------------------- MultiFieldList Properties
    	 templateString: statusAutoCompTmp,

        autocomplete: tui.flights.status.FlyToAutoComplete,
        ...
        createDownArrow: function(){
        	...
        	if(!_this.hasDownArrow){
        		...
        		on(_this.downArrow,"click",function(){
        			var flightNum = dijit.byId("flightNumber").getLabel().replace(" ","");
                	var depAirPortCode = dijit.byId("flyFromStat").getValue();
        			_this.autocomplete.targetURL =  dojoConfig.paths.webRoot + "/arrivalAutoSuggestData?depAirPortCode="+ depAirPortCode +"&flightNumber="+ flightNum +"&event=arrival";
    ...
    return tui.flights.status.FlyToAutoSuggest;
});