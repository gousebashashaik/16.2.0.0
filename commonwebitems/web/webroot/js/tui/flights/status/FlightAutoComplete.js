define("tui/flights/status/FlightAutoComplete", [
  "dojo",
  "tui/widget/_TuiBaseWidget",
  "tui/widget/mixins/AutoCompleteable"
], function(dojo) {

  dojo.declare("tui.flights.status.FlightAutoComplete", [tui.widget._TuiBaseWidget, tui.widget.mixins.AutoCompleteable], {
	...
	createQuery: function (element, event) {
	    ...
	    if (autoCompleteable.searchQuery) {
	      var query = "?arrAirPortCode="+ flyToVal + "&flightNumber=" + flightNumVal +"&"+autoCompleteable.searchProperty + "=" + element.value +"&event=departure";//["?", autoCompleteable.searchQuery, "=", element.value].join("");
	      return query;
	    }
	    return null;
	},
  ...
  return tui.flights.status.FlightAutoComplete;
});
