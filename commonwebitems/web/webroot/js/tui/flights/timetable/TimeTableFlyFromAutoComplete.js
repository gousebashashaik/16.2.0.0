define("tui/flights/timetable/TimeTableFlyFromAutoComplete", [
  "dojo",
  "dojo/io-query",
  ...
  dojo.declare("tui.flights.timetable.TimeTableFlyFromAutoComplete", [tui.widget._TuiBaseWidget, tui.widget.mixins.AutoCompleteable, tui.flights.widget.MonthPullDown], {
	...
	onType: function(element, event) {
	      ...
	      if(dojo.trim(element.value).length >= _this.charNo) {
	    	  _this.targetURL = dojoConfig.paths.webRoot + '/ws/flyfromautosuggest';
  ...
  return tui.flights.timetable.TimeTableFlyFromAutoComplete;
});
