 ...
 * 
 */
public class PassportsAndVisasComponentControllerTest
{
	...
	private LocationData getDummyLocationDataForCountry()
	{
		...
		passportAndVisaInfo.add("passportAndVisa Information for spain ");
		final Map<String, List<Object>> featureValues = new HashMap<String, List<Object>>();
		featureValues.put("passportAndVisaInfo", passportAndVisaInfo);
	...
	public void testViewPassportsAndVisasComponentForCountry()
	{
		...
		when(facade.getPassportAndVisaEditorial(location.getCode())).thenReturn(locationData);
		assertThat(locationData.getFeatureCodesAndValues().get("passportAndVisaInfo").get(0).toString(),
				is("passportAndVisa Information for spain "));
	...
	public void testViewPassportsAndVisasComponentForSubCategories()
	{
		...
		when(facade.getPassportAndVisaEditorial(location1.getCode())).thenReturn(locationData);
		assertThat(locationData.getFeatureCodesAndValues().get("passportAndVisaInfo").get(0).toString(),
				is("passportAndVisa Information for spain "));
