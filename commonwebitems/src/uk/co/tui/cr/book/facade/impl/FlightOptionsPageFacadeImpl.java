...
import uk.co.tui.cr.book.populators.AirportDataConvertor;
import uk.co.tui.cr.book.populators.CalendarViewPopulator;
...
import uk.co.tui.cr.book.view.data.teasers.AirportTeaserViewData;
...
public class FlightOptionsPageFacadeImpl implements FlightOptionsPageFacade
{
   ...
   /** The Constant TUI_SYSTEM_EXCEPTION. */
   ...
   /** The duration search service locator. */
   @Resource
   ...
   @Resource(name = "crFlightFilterCriteriaPopulator")
   ...
   /** The flight extras container populator. */
   ...
   /** The cr airport data convertor. */
   @Resource
   private AirportDataConvertor crAirportDataConvertor;
   ...
   public FlightOptionsViewData renderFlightOptions() throws TUIBusinessException
   {
       ...
       * 
       * Temporarily adding condition to work following block of code to be executed when TRACS
       * inventory as @com inventory doesn't have anite cache ready Once everything ready we need
       * remove the condtion and make the code as earlier
       */
      ...
      if (sessionService.getAttribute(EndecasearchConstants.MAIN_SEARCH_CRITERIA) != null)
      {
         ...
         populateSearchCriteria(mainSearchCriteria.getUkAirports(), searchCriteria);
      }
      else
      {
      ...
      try
      {
         alternatePackages =
            alternateFlightsSearchService.getAlternateFlightsLite(getSearchCriteriaFromSession(),
               packageModel, getDurationsToSearchAndUpdateStore(packageModel));
      }
      catch (final BookServiceException e)
      {
   ...
   /**
    * Gets the search criteria from session.
    *
    * @return the search criteria from session
    */
    ...
    *
    * @param packageModel the package model the alternative flight store
   ...
   /**
    * Render calendar overlay view for fnc.
    *
    * @param cartPackage the cart package
    * @param packages the packages
    * @param airports the airports
    * @param filterCriteria the filter criteria
    * @return the calendar view data
    */
   private CalendarViewData renderAvailibilityCalendarOverlay(final BasePackage cartPackage,
      final List<BasePackage> packages, final FlightFilterCriteria filterCriteria)
   {
      ...
      {
         highlights = packageAlternateService.resolvePackageExtraFacilities(packages);
      ...
      return calendarViewPopulator.populateCalendarViewData(packages, highlights, cartPackage,
         filterCriteria, getSearchCriteriaFromSession());
   }
    ...
    * @param packages the packages
    */
   private void sortPackagesByPrice(final List<BasePackage> packages)
   {
      Collections.sort(packages, new Comparator<BasePackage>()
      {
         @Override
         public int compare(final BasePackage package1, final BasePackage package2)
         {
            return package1.getPrice().getAmount().getAmount()
               .compareTo(package2.getPrice().getAmount().getAmount());
         }
      });
   }
    ...
    *
    * @param airportCodes the airports data
    * @param searchCriteria the search criteria
    */
   private void populateSearchCriteria(final List<String> airportCodes,
      final SearchCriteria searchCriteria)
   {
      searchCriteria.setAirports(crAirportDataConvertor.fetchAirportDetails(airportCodes));
   }
   ...
   public FlightOptionsViewData renderAvailabilityCalendar(final FlightFilterCriteria filterCriteria)
   {
      ...
      flvViewData.setCalendarViewData(renderAvailibilityCalendarOverlay(selectedPackage,
         alternativeFlightStore.getAlternates(filterCriteria.getSelectedDepartureAirport()),
         filterCriteria));
      return flvViewData;
   }
   ...
   /**
    * Populate third party flight view data.
    *
    * @param alternativeFlightPackges the alternative flight packges
    * @param packageModel the package model
    * @param viewData the view data
   ...
   /**
    * Gets the alternate flight packages.
    *
    * @param packageModel the package model
   ...
   /**
    * Fetch alternate flight store.
    *
    * @param packageModel the package model
    * @return the alternative flight store
    */
   ...
   /**
    * Gets the cabin class from package.
    *
    * @return the cabin class from package
    */
