...
import uk.co.tui.cr.book.facade.RoomOptionsPageFacade;
...
public class RoomOptionsPageFacadeImpl implements RoomOptionsPageFacade
{
   ...
   @Resource(name = "crRoomStaticContentViewDataPopulator")
   ...
   @Resource(name = "crRoomOptionsContentViewDataPopulator")
   ...
   private static final String WEBANALYTICS = "cr_webAnalyticsData";
   
   /** The Constant LAI_WARNING_MESSAGE. */
   private static final String LAI_WARNING_MESSAGE = "cr_room_lai_warning_message";
   ...
   public RoomAndBoardOptionsPageViewData getAlternateRooms()
   {
      ...
      // Reset Fixed centre(Cruise), package supplement prices to zero on load of page.
      ...
      updateBoardBasisToRoomSearchResponse(roomSearchResponse, packageModel);
   ...
   /**
    * @param roomSearchResponse
    * @param packageModel
    */
   private void updateBoardBasisToRoomSearchResponse(final RoomSearchResponse roomSearchResponse,
      final BasePackage packageModel)
   {
      final String boardBasisCode =
         packageComponentService.getHotel(packageModel).getRooms().get(0).getBoardBasis().getCode();
      for (final Map.Entry<String, List<AltRoom>> alternateRooms : roomSearchResponse
         .getAlternateRooms().entrySet())
      {
         updateSelectedBoardBasisToRoomRespones(boardBasisCode, alternateRooms);
      }
      
   }
   
   private void updateSelectedBoardBasisToRoomRespones(final String boardBasisCode,
      final Map.Entry<String, List<AltRoom>> alternateRooms)
   {
      for (final AltRoom room : alternateRooms.getValue())
      {
         for (final BoardBasis bb : room.getBoardBasis())
         {
            if (bb.getCode().equalsIgnoreCase(boardBasisCode))
            {
               bb.setSelected(true);
            }
            else
   ...
   {
      final Stay fixedCentre = packageComponentService.getCruise(packageModel);
      if (fixedCentre != null)
      {
         for (final Room room : fixedCentre.getRooms())
         {
    ...
    * (non-Javadoc)
    *
    * @see uk.co.tui.cr.book.facade.RoomOptionsPageFacade#populateRoomOptionsAllView
    * (uk.co.tui.book.domain.lite.BasePackage,
   ...
   {
      packageExtraFacilityFacade.populateRoomOptionsAllView(packageModel, roomOptionsViewData);
   }
   ...
   private List<RoomViewData> getRoomViewDataForEachRoom(final List<AltRoom> roomsList)
   {
      ...
      final Integer duration =
         Integer.valueOf(packageComponentService.getHotel(getPackageModel()).getDuration());
   ...
   private List<BoardBasisViewData> populateBoardBasis(final AltRoom room)
   {
      ...
      for (final BoardBasis bb : room.getBoardBasis())
      {
         ...
         perPersonPerNightPrice =
            alternateRoomsService.populatePerPersonPerNightPrice(room, priceDifference,StayType.HOTEL);
    ...
    * (non-Javadoc)
    *
    * @see uk.co.tui.cr.book.facade.RoomOptionsPageFacade#updateSelectedRoomAllocation
    * (uk.co.tui.cr.book.criteria.RoomSelectionCriteria)
    */
   @Override
    ...
    * (non-Javadoc)
    *
    * @see uk.co.tui.cr.book.facade.RoomOptionsPageFacade#populateRoomsForWebAnalytics
    * (uk.co.tui.cr.book.view.data.RoomAndBoardOptionsPageViewData)
    */
   @Override
      ...
      final RoomAndBoardOptionsPageViewData roomAndBoardOptionsPageViewData)
   {
      ...
      final List<RoomViewData> roomViewDataList =
         roomAndBoardOptionsPageViewData.getPackageViewData().getAccomViewData().get(indexval)
            .getRoomViewData();
   ...
   {
      final List<Stay> stay = packageComponentService.getAllStays(packageModel);
      int indexval = 0;
      for (final Stay stayvar : stay)
      {
         if (stayvar.getType() != StayType.SHIP)
         {
            indexval = stay.indexOf(stayvar);
         }
      }
      return indexval;
   }
    ...
    * (non-Javadoc)
    *
    * @see uk.co.tui.cr.book.facade.FlightOptionsPageFacade#updatePackageIntoCart
