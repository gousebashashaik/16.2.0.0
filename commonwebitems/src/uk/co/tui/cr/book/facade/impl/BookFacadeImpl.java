...
import uk.co.tui.cr.book.facade.BookFacade;
import uk.co.tui.cr.book.populators.AlertPassengerViewDataPopulator;
import uk.co.tui.cr.book.populators.AlertPopulator;
import uk.co.tui.cr.book.populators.BookingDetailsViewDataPopulator;
...
public class BookFacadeImpl implements BookFacade {
    ...
    @Resource(name = "crAlertPassengerViewDataPopulator")
    private AlertPassengerViewDataPopulator alertPassengerViewDataPopulator;

    @Resource(name = "crBookingDetailsViewDataPopulator")
     ...
     *            the holiday package
     * @param latestPacakgeFromInventory
     *            the latest pacakge from inventory
    ...
    /**
     * This method handles the infants not yet born scenario against the
     * inventory.
     */
    @Override
    ...
    /**
     * this method checks the price and availability of the selected package
    ...
    public List<AlertViewData> checkPriceAndAvailability()
        ...
        try {
            ...
            feedBackList = checkPriceAvailabilityService
                    .updatePriceAndAvailability(basePackage,
                            InventoryType.ATCOM == basePackage.getInventory()
                                    .getInventoryType());
        ...
        alertViewDataList = alertPopulator.populateTotalCostAlert(
                alertViewDataList, feedBackList);
    ...
    private void handleBookServiceException(final BookServiceException e)
        ...
        final List<String> multicomErrorCodes = Arrays.asList(StringUtils
                .split(Config.getParameter("multicom_error_codes"), ','));
        final String errorCode = StringUtils.trim(e.getErrorCode());
        if (multicomErrorCodes.contains(errorCode)
                || StringUtils.startsWithIgnoreCase(errorCode,
                        MULTICOMERROR_PREFIX)) {
            throw new FlightSoldOutException(e.getErrorCode(), e);
        }
        LOGGER.error(TUI_EXCEPTION + e.getMessage());
        final List<String> soldOutErrorCodes = Arrays.asList(StringUtils.split(
                Config.getParameter("soldOutErrorCodes"), ','));
        if (isSoldoutError(e.getErrorCode(), soldOutErrorCodes)) {
            throw new TUIBusinessException(e.getErrorCode(),
                    e.getCustomMessage(), e);
        } else {
            throw new TUISystemException(e.getErrorCode(),
                    e.getCustomMessage(), e);
        }
    }
     ...
     */
    private boolean isSoldoutError(final String errorCode,
            final List<String> soldOutErrorCodes) {
        // Comparison logic has been moved to TibcoSoapFaultHandler
        return PackageUtilityService.isSoldOutErrorMatches(errorCode,
                soldOutErrorCodes);
    }

    /**
     * This method is called to render the confirmation page after successful
     * booking.
     *
     */
    @Override
    public BookingDetailsViewData confirmBooking() {
        ...
        bookingDetailsViewDataPopulator.populate(
                packageCartService.getBasePackage(), bookingDetailsViewData);
     ...
     *
     * @see
     * uk.co.tui.th.book.facade.BookFacade#getPaymentPageRelativeUrl(uk.co.tui.
     * domain.model.InclusivePackageModel, java.lang.String, java.util.Map)
     */
    @Override
    public String getPaymentPageRelativeUrl(final Map<String, String> urlMap,
            final String hostName, final String contextPath) throws FlightSoldOutException,
        ...
        paymentRequest.setBookingSessionIdentifier(sessionService
        ...
        sessionService.setAttribute(SessionObjectKeys.PAYMENTREQUEST,
                paymentRequest);
    ...
    private void populateMultiCentreData(final PaymentRequest paymentRequest) {
        if (packageCartService.getBasePackage().getListOfHighlights()
            ...
            paymentRequest
                    .setMultiCentreData(convertMultiCentreData(linkedItemService.getMultiCentreData(packageComponentService
                            .getStay(packageCartService.getBasePackage())
                            .getCode())));
        }
    ...
    private Map<String, List<String>> convertMultiCentreData(
        ...
        for (final MultiCentreData eachData : multiCentreData) {
            ...
            paymentData.get(location)
                    .add((String) eachData.getLocations().values().iterator()
                            .next());
            paymentData.get(duration).add(
                    new StringBuilder(eachData.getDuration()).append(nights)
                            .append(eachData.getName()).toString());
        }
        return paymentData;
    }
    ...
    private void populateAlertViewData(final PaymentRequest paymentRequest)
        ...
        try {
            ...
            LOGGER.error("TUIBusinessException", e);
        }
    ...
    private void populateWebAnalyticsData(final PaymentRequest paymentRequest) {
        ...
        final List<WebAnalytics> webAnalyticsList = sessionService
                .getAttribute("webAnalyticData");

        final List<WebAnalytics> webAnalyticsData = new ArrayList<WebAnalytics>();
        webAnalyticsData.addAll(webAnalyticsList);
        populatePromoAnalytics(webAnalyticsData);
        ...
        if (CollectionUtils.isNotEmpty(webAnalyticsData)) {
            ...
            for (final WebAnalytics webAnalytics : webAnalyticsData) {
                if (webAnalytics != null) {
                    webAnalyticsMap.put(webAnalytics.getKey(),
                            webAnalytics.getValue());
    ...
    /**
     * Populates analytics wrt Payment page
     *
     * @param webAnalyticsData
     *            current analytics list
     *
     */
    private void populatePromoAnalytics(
            final List<WebAnalytics> webAnalyticsData) {
        ...
        if (packageModel.getPromotionalDiscount() != null) {
            final PromotionalDiscount promotionalDiscount = packageModel
                    .getPromotionalDiscount();
            promoCode = promotionalDiscount.getCode();
            promoValue = String.valueOf(promotionalDiscount.getPrice()
                    .getAmount().getAmount());
            promoValue = removeMinusSymbol(promoValue);
        }

            webAnalyticsData.add(new WebAnalytics(PROMOVALUE, promoValue));
            webAnalyticsData.add(new WebAnalytics(PROMOCODE, promoCode));
    ...
    private String getPaymentPageRelativeUrl(final String[] paymentTokens) {
        final MessageFormat url = new MessageFormat(
                CommonwebitemsConstants.PAYMENT_RELATIVE_URL);
    ...
    public void flushSessionObjects() {
        final PackageExtraFacilityStore packageExtraFacilityStore = sessionService
        ...
        final CarHireUpgradeExtraFacilityStore carHireUpgradeExtraFacilityStore = sessionService
                .getAttribute("carHireUpgradeExtraFacilityStore");
