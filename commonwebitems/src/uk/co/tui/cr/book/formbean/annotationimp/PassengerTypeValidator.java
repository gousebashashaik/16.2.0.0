...
import uk.co.tui.cr.book.formbean.annotation.ValidatePassengerType;
...
public class PassengerTypeValidator
        implements
            ConstraintValidator<ValidatePassengerType, PassengerInfoFormBean> {

    /** The male title list. */
    private final List<String> maleTitleList = Arrays.asList("MR", "MSTR",
            "DR", "REV", "PROF");

    /** The female title list. */
    private final List<String> femaleTitleList = Arrays.asList("MRS", "MISS",
            "MS", "DR", "REV", "PROF");

    /** The title list. */
    private final List<String> titleList = Arrays.asList("MR", "MSTR", "MRS",
            "MISS", "MS", "DR", "REV", "PROF");
    ...
    public boolean isValid(final PassengerInfoFormBean passengerDetails,
        ...
        if (!titleList.contains(title)) {
            ...
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate(
                    PassengerDetailConstants.GENDER_MESS_CODE)
                    .addNode("gender").addConstraintViolation();
            result = false;
        } else if (checkGenderCondition(title, gender)) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate(
                    PassengerDetailConstants.WRONG_GENDER_MESS_CODE)
                    .addNode("gender").addConstraintViolation();
            result = false;
        }
        return result;
