 ...
 * @author samantha.gd
 */
public class AirportTeaserViewDataPopulator
        implements
            Populator<BasePackage, AirportTeaserViewData> {
    ...
    public void populate(final BasePackage source,
            final AirportTeaserViewData target) {
        final String airportCode = packageComponentService
                .getFlightItinerary(source).getOutBound().get(0)
                .getDepartureAirport().getCode();
        target.setAirportCode(airportCode);
        if (isNotEmpty(airportCode)) {
            target.setAirportName(airportService.getAirportByCode(airportCode)
                    .getName());
        }
