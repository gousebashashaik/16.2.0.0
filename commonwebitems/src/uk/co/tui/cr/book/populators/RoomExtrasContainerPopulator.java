 ...
 *
 */
public class RoomExtrasContainerPopulator
        implements
    ...
    private void populateLateCheckoutOptions(final ExtraFacility extras,
            final ExtraFacilityViewDataContainer target) {
        if (StringUtils.equalsIgnoreCase(extras.getExtraFacilityCode(),
                ExtraFacilityConstants.LCD)) {
    ...
    private void populateLateCheckout(final ExtraFacility extraFacilityModel,
        ...
        final List<Room> roomModelList = packageComponentService.getStay(
                packageModel).getRooms();
        final int roomModelSize = roomModelList.size();
        int noOfSelectedRooms = getQuantity(extraFacilityModel);
        final int noOfAvailableRooms = getNoOfAvlRooms(extraFacilityModel,
                roomModelSize);

        final List<ExtraFacility> pkgLCD = (List<ExtraFacility>) ExtraFacilityUtils
                .getExtrasForCategoryAndCode("Accommodation", "LCD",
        ...
        if (CollectionUtils.isNotEmpty(pkgLCD)
            ...
            updateLateCheckoutService.addLateCheckout(packageModel,
                    pkgLCD.get(0), String.valueOf(noOfAvailableRooms));
            noOfSelectedRooms = noOfAvailableRooms;
        }

        final ExtraFacilityCategoryViewData lateChkotCategoryViewData = target
                .getLateCheckOut();
        lateChkotCategoryViewData
                .setExtraFacilityCategoryCode(extraFacilityModel
                        .getExtraFacilityCategory().getCode());
        lateChkotCategoryViewData
                .setExtraFacilityGroupCode(ExtraFacilityConstants.LCD);
        ...
        extraViewData.setGroupCode(extraFacilityModel.getExtraFacilityGroup()
                .toString());
        ...
        populateLCDLAI(
                packageExtrasServiceLite.getLateCheckOutThresoldValues(),
                extraViewData);
     ...
     */
    @SuppressWarnings(BOXING)
    private int getNoOfAvlRooms(final ExtraFacility extraFacilityModel,
            final int roomModelSize) {
     ...
     *
     * @param lateCheckOutThreshold
     *            the late check out threshold
     * @param extraViewData
     *            the extra view data
     */
    private void populateLCDLAI(final int lateCheckOutThreshold,
        ...
        if (extraViewData.getAvailableQuantity() <= lateCheckOutThreshold) {
            ...
            extraViewData.setLimitedAvailabilityText(fetchStaticContent().get(
                    ROOMS_LA_TEXT).toString());
        }
    }
