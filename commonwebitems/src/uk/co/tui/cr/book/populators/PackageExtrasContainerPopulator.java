...
import uk.co.tui.cr.book.content.populators.ExcursionContentViewDataPopulatorLite;
   ...
   Populator<List<ExtraFacility>, ExtraFacilityViewDataContainer>
{
   ...
   @Resource(name = "crExcursionContentPopulatorLite")
      ...
      final ExtraFacilityViewDataContainer target)
   {
      ...
      for (final ExtraFacility extras : source)
      {
         ...
         populateClassActWorkshopExtrasOptions(extras, target, exFacilityCategoryCode);
    ...
    */
   private void populateClassActWorkshopExtrasOptions(final ExtraFacility extras,
    ...
    * @param target
    */
   private void populateClassActWorkshopExtras(final ExtraFacility extraFacilityModel,
      final ExtraFacilityViewDataContainer target)
   {
      ...
      final int paxCount =
         PassengerUtils.getPassengersWithAgeRang(restrictions.getMinAge(),
            restrictions.getMaxAge(), packageModel.getPassengers()).size();

      if (checkExtraFacility(extraFacilityModel, target.getWorkshopOptions(), packageModel))
      {
         return;
      }

      populateClassActWorkshopViewData(extraFacilityModel, target, selectedQuantity, paxCount);
    ...
    */
   private void populateClassActWorkshopViewData(final ExtraFacility extraFacilityModel,
      final ExtraFacilityViewDataContainer target, final int selectedQuantity, final int paxCount)
   {

      final int avlQuantity = extraFacilityModel.getQuantity();

      final ExtraFacilityCategoryViewData workshopExtraFacilityViewData =
         target.getWorkshopOptions();
      workshopExtraFacilityViewData.setExtraFacilityCategoryCode(extraFacilityModel
         .getExtraFacilityCategory().getCode());
      workshopExtraFacilityViewData.setExtraFacilityGroupCode(extraFacilityModel
      ...
      extraFacilityViewData.setCurrencyAppendedPerPersonPrice(getCurrencyAppendedPrice(unitPrice,
         currency));

      extraFacilityViewData
         .setCurrencyAppendedPrice(getCurrencyAppendedPrice(totalPrice, currency));

      extraFacilityViewData.setCurrencyAppendedPricePerClass(getCurrencyAppendedPrice(unitPrice,
         currency));
      ...
      workshopExtraFacilityViewData.getExtraFacilityViewData().add(extraFacilityViewData);
      ...
      final ExtraFacilityViewDataContainer target)
   {
      ...
      if (checkExtraFacility(extraFacilityModel, target.getInfantOptions(), packageModel))
      {
         return;
      }
    ...
    *
    * @param extraFacilityModel the extra facility model
    * @param extraFacilityCategoryViewData the target
    * @param packageModel the package model
    * @return true, if successful
    */
   @SuppressWarnings(BOXING)
