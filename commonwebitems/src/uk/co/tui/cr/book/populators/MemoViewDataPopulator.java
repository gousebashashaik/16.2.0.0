 ...
 * @author samantha.gd
 */
public class MemoViewDataPopulator
        implements
    ...
    public void populate(final BasePackage source, final PackageViewData target)
        ...
        // SONAR FIX-Changing logic for "null" check (to avoid parameter
        // reassignment in actual population)
     ...
     */
    public void populate(final BasePackage source, final MemoViewData target)
            throws ConversionException {
        ...
        if (memoModelList != null && CollectionUtils.isNotEmpty(memoModelList)) {
            for (final Memo memomodel : memoModelList) {
                description.add(String.valueOf(memomodel.getDescription()));
            }
