...
import uk.co.tui.cr.book.view.data.teasers.DepartureDateTeaserViewData;

/**
 * @author samantha.gd
 *
 */
public class DateTimeTeaserViewDataPopulator
        implements
            Populator<BasePackage, DepartureDateTeaserViewData> {
    ...
    public void populate(final BasePackage source,
            final DepartureDateTeaserViewData target) {
        final Date dateTime = (packageComponentService
                .getFlightItinerary(source)).getOutBound().get(0).getSchedule()
                .getDepartureDate();
        ...
        target.setDisplayDate(formattedDate.substring(0, formattedDate.length()
                - DATE_OFFSET));
