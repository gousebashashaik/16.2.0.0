...
import uk.co.tui.book.populators.ExtraFacilityViewDataPopulator;
import uk.co.tui.book.populators.InsuranceViewDataPopulatorLite;
...
import uk.co.tui.book.view.data.BaggageExtraFacilityViewData;
...
import uk.co.tui.book.view.data.ExtrasToPassengerRelationViewData;
...
import uk.co.tui.book.view.data.PassengerViewData;
import uk.co.tui.book.view.data.PaxCompositionViewData;
...
public class ExtraFacilityUpdator
{
      ...
      final ExtraFacilityCategoryViewData categoryViewData)
   {
      for (final ExtraFacilityViewData extraView : categoryViewData.getExtraFacilityViewData())
      {
         ...
         extraView.setCurrencyAppendedPrice(getCurrencyAppendedPriceWithDecimalPart(amount));
      }
      ...
      final ExtraFacilityCategoryViewData categoryViewData, final ExtraFacility extraModel)
   {
               ...
               extraFacilityViewData
                  ...
                  .multiply(
                     BigDecimal.valueOf(extraFacilityViewData.getPassengerExtraFacilityMapping()
                        .getPassengers().size())).setScale(TWO, RoundingMode.HALF_UP),
      ...
      if (!isExtraPresent)
      {
         ...
         bagView.setCurrencyAppendedPrice(getCurrencyAppendedPrice(
            bagView
               .getPrice()
               .multiply(
                  BigDecimal.valueOf(bagView.getPassengerExtraFacilityMapping().getPassengers()
                     .size())).setScale(TWO, RoundingMode.HALF_UP),
            currencyResolver.getSiteCurrency()));
      ...
      final ExtraFacility extraModel, final boolean isExtraPresent)
   {
      if (!isExtraPresent)
      {
         if (extraModel instanceof InsuranceExtraFacility)
         {
            ...
            insuranceViewDataPopulatorLite.populate(insuranceModel, insViewData);
      ...
      final ExtraFacilityViewData extraView)
   {
      ...
      if (isKidsActivityExtra(extraView) && BigDecimal.ZERO.compareTo(extraView.getPrice()) == 0)
      {
         ...
         extraView.setCurrencyAppendedPrice(getCurrencyAppendedPrice(
            amount.setScale(TWO, RoundingMode.HALF_UP), currencyResolver.getSiteCurrency()));
      }
   }
      ...
      final ExtraFacilityViewData extraFacilityViewData)
   {
      ...
      {
         updateSeatExtraFacilityQuantity(getPackageModel());
      ...
      final ExtraFacilityCategoryViewData excursionCategory)
   {
      ...
      excursionCategory
         .setCurrencyAppendedCategoryTotalPrice(getCurrencyAppendedPriceWithDecimalPart(totalPrice));
   ...
   public void updateSeatExtrasRelativePrice(final FlightOptionsViewData viewData)
   {
      ...
      selectedSeatPrice = getSelectedSeatPrice(viewData, selectedSeatPrice);
      ...
      for (final ExtraFacilityViewData viewExtra : extras)
      {
         ...
         if (StringUtil.equalsIgnoreCase(viewExtra.getCode(), ExtraFacilityConstants.PREMIUM_SEAT))
         {
            ...
            viewExtra
               .setCurrencyAppendedPrice(getCurrencyAppendedPrice(premiumPrice, currencyCode));
            viewExtra.setCurrencyAppendedPerPersonPrice(getCurrencyAppendedPrice(
               getCalculatedPrice(
                  price,
                  new BigDecimal(PassengerUtils.getChargeablePaxCount(getPackageModel()
                     .getPassengers()))), currencyCode));
         }
         else
         {

            viewExtra.setPrice(price.subtract(selectedSeatPrice));
            viewExtra.setCurrencyAppendedPrice(getCurrencyAppendedPrice(viewExtra.getPrice()
               .setScale(TWO, RoundingMode.HALF_UP), currencyCode));
            final int chargeablePersons = noOfApplicableAdults + noOfApplicableChildren;
            viewExtra.setCurrencyAppendedPerPersonPrice(getCurrencyAppendedPrice(
               getCalculatedPrice(viewExtra.getPrice(), BigDecimal.valueOf(chargeablePersons)),
               currencyCode));
         }
      }
   ...
   private BigDecimal getSelectedSeatPrice(final FlightOptionsViewData viewData,
      final BigDecimal selectedSeatPrice)
   {
      BigDecimal selSeatPrice = selectedSeatPrice;
         ...
         .getExtraFacilityCategoryViewData())
      {
            ...
            ExtraFacilityConstants.SEAT_EXTRAS_CATEGORY_CODE))
         {
            ...
            selSeatPrice = getSelectedSeatExtraPrice(categoryViewData);
         }
      ...
      final List<ExtraFacility> cpoExtraFacilities)
   {
      ...
      {
         LOGGER.error("Caught QueryExecutionException", qpe);
         return Collections.emptyMap();
      }
      catch (final QueryExecutionException qee)
      {
         LOGGER.error("Caught QueryExecutionException", qee);
         return Collections.emptyMap();
      }
      ...
      final List<ExtraFacilityViewData> extras, final BigDecimal selectedSeatPrice)
   {
      for (final ExtraFacilityViewData viewExtra : extras)
      {
         if (isValidCode(seatExtrasCode, isPremiumUpdated, viewExtra))
         {
            ...
            viewExtra.setCurrencyAppendedPrice(getCurrencyAppendedPrice(viewExtra.getPrice()
               .setScale(TWO, RoundingMode.HALF_UP), currencyResolver.getSiteCurrency()));
            final int chargeablePersons = noOfApplicableAdults + noOfApplicableChildren;
            viewExtra.setCurrencyAppendedPerPersonPrice(getCurrencyAppendedPrice(
               getCalculatedPrice(viewExtra.getPrice(), BigDecimal.valueOf(chargeablePersons)),
               currencyResolver.getSiteCurrency()));
      ...
      final BigDecimal totalPrice, final ExtraFacility eachExtraFacility)
   {
         ...
         eachExtraFacility.getExtraFacilityCategory().getCode()))
      {
         ...
         categoryViewData
            .setCurrencyAppendedCategoryTotalPrice(getCurrencyAppendedPriceWithDecimalPart(price));
      }
      return price;
   }
         ...
         ExtraFacilityConstants.SWIMACADEMY))
      {
         if (checkIsPassengerPresent(packageExtraFacility, passenger.getIdentifier()))
         {
            containerExtraFacility.setSelectedQuantity(packageExtraFacility.getPrices()
               .get(getPriceIndexBasedOnExFacilityType(packageExtraFacility)).getQuantity()
               .intValue());
            containerExtraFacility.setCurrencyAppendedTotalPrice(getCurrencyAppendedPrice(
               packageExtraFacility.getPrices()
                  .get(getPriceIndexBasedOnExFacilityType(packageExtraFacility)).getAmount()
                  .getAmount(), currencyResolver.getSiteCurrency()));
    ...
    * @return
    */
   @SuppressWarnings({ "javadoc", BookFlowConstants.BOXING })
   ...
   {
      Map<String, String> summaryContentMap = null;
      summaryContentMap = staticContentServ.getSummaryContents();
      ...
      final PackageViewData packageViewData)
   {
      ...
      if (transferOptionOrCarHireNotExits(packageModel) && isTransferPresent(highlights))
      {
         ...
         if (isCarHirePresent(highlights))
         {
            handleCarHire(packageModel, transferViewData);
         }
   ...
   /**
    * @param packageModel
    * @param transferViewData
    */
   private void handleCarHire(final BasePackage packageModel,
      final ExtraFacilityViewData transferViewData)
   {
      boolean noCarHireOpted = false;
      final CarHireUpgradeExtraFacilityStore carHireUpgradeExtraFacilityStore =
         (CarHireUpgradeExtraFacilityStore) sessionService
            .getAttribute(SessionObjectKeys.CARHIRE_UPGRADE_EXTRA_FACILITY_STORE);
      // Check if free car hire is rejected
      if (SyntacticSugar.isNotNull(carHireUpgradeExtraFacilityStore)
         && CollectionUtils.isNotEmpty(carHireUpgradeExtraFacilityStore
            .getExtraFacilityLite(packageModel.getId())))
      {
         // Free car hire requested and user has rejected the free
         // car hire option
         noCarHireOpted = true;
      }
      if (noCarHireOpted)
      {
         transferViewData.setSummaryDescription(getDescription(ExtraFacilityConstants.NOTF));
         transferViewData.setCode(ExtraFacilityConstants.NOTF);
         transferViewData.setSelection(StringUtils.EMPTY);
      }
   }
    ...
    * @param highlights
    */
   private boolean isCarHirePresent(final List<HighLights> highlights)
   {
      return CollectionUtils.isNotEmpty(highlights)
         && highlights.contains(HighLights.FREE_CAR_HIRE);
   }
