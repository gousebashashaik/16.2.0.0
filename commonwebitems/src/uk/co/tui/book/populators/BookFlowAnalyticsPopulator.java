...
import uk.co.tui.book.view.data.WebAnalyticsALTFLTData;
...
public class BookFlowAnalyticsPopulator implements Populator<Object, Map<String, WebAnalytics>>
{
   ...
   /** Web analytics data */
   private static final String WEBANALYTICSDATA = "fc_webAnalyticsData";
   ...
   /**
    * The value of the parameter will be ‘Debit’, ‘Credit’ or whatever other payment method was used
    *
    * @param analyticMap
    */
   ...
   private void populateLowDeposit(final Map<String, WebAnalytics> analyticMap)
   {
      ...
      {
         for (final Deposit deposit : packageModel.getDeposits())
         {
   ...
   private boolean isLowDepositAvailable(final Deposit deposit)
   {
   ...
   private boolean populateifStandardDepositAvailable(final Map<String, WebAnalytics> analyticMap,
      final Deposit deposit, final boolean lowDepositAvailable)
   {
      int minPay = 0;
    ...
    */
   private boolean isStandardDepositAvailable(final Deposit deposit)
   {
      ...
      final PackageViewData packageViewData)
   {
      ...
      rmSlt = getRmSltValue(packageViewData, rmSltComp, defaultRooms);
    ...
    * @return
    */
   private String getRmSltValue(final PackageViewData packageViewData,
      ...
      final StringBuilder rmSltComp, final List<RmSltViewData> defaultRooms)
   {
         ...
         .getRoomViewData())
      {
         ...
         constructRmsltDescAndPriceIndependetly(roomViewData, isPresentInPackageModel,
            rmlSltRoomDesc, rmlSltRoomPrice);
      }
   ...
   private void constructRmsltDescAndPriceIndependetly(final RoomViewData roomViewData,
      final boolean isPresentInPackageModel, final StringBuilder rmlSltRoomDesc,
      final StringBuilder rmlSltRoomPrice)
   {
   ...
   /**
    * Valid web analytics data.
    *
    * @param webAnalyticsData the web analytics data
    * @return true, if valid web analytics data
    */
   private boolean validWebAnalyticsData(final WebAnalyticsData webAnalyticsData)
   {
      return SyntacticSugar.isNotNull(webAnalyticsData)
   ...
   private void populateAltFlt(final Map<String, WebAnalytics> analyticMap)
   {
      String altFlt = NULL;
      final WebAnalyticsData webAnalyticsData = sessionService.getAttribute(WEBANALYTICSDATA);
      if (validWebAnalyticsData(webAnalyticsData))
      {
   ...
   /**
    * Checks if room count is below the threshold.
    *
    * @param selectedRoom the selected room
    * @return true, if is limited room
    */
   private boolean isLimitedRoom(final Room selectedRoom)
   {
      if (selectedRoom != null && selectedRoom.getQuantity() != null)
      {
         return selectedRoom.getQuantity().intValue() <= getLimitedAvailabilityThreshold();
      }
      return false;
   }

   private int getLimitedAvailabilityThreshold()
   {
      return bookFlowTuiConfigService.getAvailableRoomsThreshold();
   }
   ...
   private void populateBoard(final Map<String, WebAnalytics> analyticMap)
   {
      String board = NULL;
      final BasePackage packageModel = getCartPackageModel();
      if (packageComponentService.getStay(packageModel) != null)
      {
         final Collection<Room> selectedRooms =
            packageComponentService.getStay(packageModel).getRooms();
      ...
      final PackageViewData packageViewData)
   {
      if (CollectionUtils.isNotEmpty(packageViewData.getFlightViewData().get(0)
         .getOutboundSectors()))
      {
         final List<uk.co.tui.book.view.data.Leg> outbounds =
   ...
   {
      constructTuiAncCompIfExtraFacility(tuiAncComp, extraFacility);
   }
    ...
    * @param extraFacility
    */
   private void constructTuiAncCompIfExtraFacility(final StringBuilder tuiAncComp,
      final ExtraFacility extraFacility)
   {
