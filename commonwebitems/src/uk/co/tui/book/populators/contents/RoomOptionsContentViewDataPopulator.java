 ...
 *
 */
public class RoomOptionsContentViewDataPopulator
        implements
     ...
     *            the target
     */
    private void populateAccommodationContentMap(
            final ExtraFacilityCategory category, final ContentViewData target) {
        for (final ExtraFacility extra : category.getExtraFacilities()) {
            final List<DynamicContentConfigModel> dynamicContents = genericContentService
                    .getDynamicContentConfig(category.getInventoryCode(),
                            extra.getExtraFacilityCode(),
                            extra.getCorporateCode());
