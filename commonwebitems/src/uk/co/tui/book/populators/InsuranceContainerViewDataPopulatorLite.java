   ...
   Populator<List<ExtraFacilityCategory>, InsuranceContainerViewData>
{
      ...
      final ExtraFacilityCategory categoryModel, final InsuranceContainerViewData target)
   {
      if (CollectionUtils.isNotEmpty(categoryModel.getExtraFacilities()))
      {
         for (final ExtraFacility extraFacility : categoryModel.getExtraFacilities())
         {
            ...
            // To do :- Temporary fix
            populateInsurancePaxViewData(insExtraFacility, insPassViewDataList);
      ...
      for (final Passenger passenger : packageModel.getPassengers())
      {
         if (!checkIfDesktop())
         {
            if (passenger.getType() != PersonType.INFANT)
            {
               ...
               populateInsPassViewData(insPassViewData, adultCount, passenger);
    ...
    * @param passenger
    */
   private void populateInsPassViewData(final InsurancePassengerViewData insPassViewData,
   ...
   private int getAdultCount()
   {
	...
	{
		final List<Passenger> passengers = packageCartService.getBasePackage().getPassengers();
		return  PassengerUtils.getPersonTypeCountFromPassengers(passengers, EnumSet.of(PersonType.CHILD));
	}
}
