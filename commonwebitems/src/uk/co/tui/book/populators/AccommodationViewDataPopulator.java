...
import uk.co.tui.book.view.data.SummaryRoomViewData;
...
public class AccommodationViewDataPopulator implements Populator<BasePackage, PackageViewData>
{
   ...
   private void populateAccomViewData(final BasePackage source, final AccommodationViewData target)
   {
      ...
      if (availableaccomm != null)
      {
         ...
         target.setBoardBasisName(boardBasisProperty.getValue(rooms.get(0).getBoardBasis()
            .getCode()
            + ".value"));
         // some Accommodation/villas may not have official rating.
         setRating(target, availableaccomm);
    ...
    */
   private void setRating(final AccommodationViewData target, final Stay availableaccomm)
   {
    ...
    * @param location
    * @return true location type matches.
    */
   ...
   private void populateRoomViewData(final Stay availableaccomm, final AccommodationViewData target)
   {
      ...
      for (final Room room : availableaccomm.getRooms())
      {
         ...
         else
         {
            ...
            roomViewData.setCurrencyAppendedPrice(String.valueOf(room.getRoomUpgradePrice()
               .getAmount().getAmount().setScale(TWO, RoundingMode.HALF_UP)));
         }
   ...
   private void populatePrice(final Room room, final RoomViewData roomViewData)
   {
      if (null != room.getPrice() && null != room.getPrice().getAmount())
      {
         ...
         roomViewData.setCurrencyAppendedPrice(String.valueOf(room.getPrice().getAmount()
            .getAmount().setScale(TWO, RoundingMode.HALF_UP)));
      }
   }
