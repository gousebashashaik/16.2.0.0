...
public class StaticContentImportContext {
    ...
    public void prepareStaticContentData(final Map<Integer, String> line) {
        ...
        if (StringUtil.notEmptyAndNotBlank(uniqueId)
                && StringUtil.notEmptyAndNotBlank(value)) {
            ...
            line.put(getKey(KEY_NINE_COMP_DESC),
                    getValue(line, CVS_COL_THREE_COMPONENT_DESC));
            line.put(getKey(KEY_TEN_VALUE), StringEscapeUtils
                    .unescapeXml(StringEscapeUtils.unescapeHtml(value)));
            line.put(getKey(KEY_TEN_COMP_ID),
                    getValue(line, CVS_COL_FIVE_COMPONENT_ID));
        } else {
     ...
     */
    private void checkForDuplicateKey(final String uniqueId,
            final Map<String, String> duplicateFinder) {
        if (duplicateFinder.containsKey(uniqueId)) {
            LOGGER.error("*********************************************************************");
            LOGGER.error("******Found the duplicate key while importing static Content data:::"
                    + uniqueId);
            LOGGER.error("**********************************************************************");
        } else {
