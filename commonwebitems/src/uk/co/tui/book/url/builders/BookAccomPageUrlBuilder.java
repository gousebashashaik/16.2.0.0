 ...
 *
 */
package uk.co.tui.book.url.builders;
...
public class BookAccomPageUrlBuilder
{
   ...
   public String getAccomBookFlowPageUrl(final String tab)
   {
      ...
      if (searchParameterInSession != null)
      {
         ...
         accomPageUrl.append(BRAND_TYPE).append(EQUALS)
            .append(searchParameterInSession.getBrandType()).append(AND);
         accomPageUrl.append(setRoomsData(searchParameterInSession));
      ...
      else
      {
         ...
         accomPageUrl.append(FROM).append(EQUALS).append(getDepartureAirportCode(outBoundLegList))
            .append(AND);
         accomPageUrl.append(WHERE_TO).append(EQUALS).append(stay.getCode());
      ...
      setDisplayPriceURL(accomPageUrl);
   ...
   private void setDisplayPriceURL(final StringBuilder accomURL)
   {
      final String displayPrice = sessionService.getAttribute(DISPLAYPRICE);
      if (!StringUtils.isEmpty(displayPrice))
      {
         accomURL.append(AND).append(PRICE).append(EQUALS).append(displayPrice);
      }
   }
   ...
   private String setRoomsData(final SearchResultsRequestData searchParameter)
   {
      ...
      {
         room.append("room").append(EQUALS);
         
         for (int i = 0; i < searchParameter.getRooms().size(); i++)
         {
            final StringBuilder party = new StringBuilder();
            
            final List<Integer> ages = searchParameter.getRooms().get(i).getChildrenAge();
            for (final int age : ages)
            {
               party.append(age).append(",");
            }
            
            final StringBuilder noOfParty =
               new StringBuilder().append(i + 1).append("|")
                  .append(searchParameter.getRooms().get(i).getNoOfAdults()).append("|")
                  .append(searchParameter.getRooms().get(i).getNoOfSeniors()).append("|")
                  .append(searchParameter.getRooms().get(i).getNoOfChildren()).append("|")
                  .append(searchParameter.getRooms().get(i).getInfantCount()).append("|")
                  .append(party).append("-");
