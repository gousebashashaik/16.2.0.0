...
import uk.co.tui.book.facade.RoomOptionsPageFacade;
...
import uk.co.tui.book.populators.RoomViewDataPopulator;
...
import uk.co.tui.book.view.data.RmSltViewData;
...
import uk.co.tui.book.view.data.RoomOptionsViewData;
...
public class RoomOptionsPageFacadeImpl implements RoomOptionsPageFacade
{
   ...
   private static final String WEBANALYTICS = "fc_webAnalyticsData";
   
   /** The Constant LAI_WARNING_MESSAGE. */
   private static final String LAI_WARNING_MESSAGE = "fc_room_lai_warning_message";
    ...
    */
   @Override
   public RoomAndBoardOptionsPageViewData getAlternateRooms()
   {
      final BasePackage packageModel = getPackageModel();
      
      // this will remove the promotional code from the product in backward
      // flow
      promotionalCodeValidationService.removePromotionalCode(getPackageModel());
   ...
   {
      // roomAndBoardOptionsPageViewData.setAccomViewData(accomodationFacade.getRoomsForAccommodation(packageCode,
   ...
   {
      // DE33236:PAT_FC_TRACS_Price is not getting updated after upgrading
      // room in RO page.
      // For the MSC criteria we are removing.
   ...
   /**
    * @return itemSearchResponse itemSearchResponse
    */
   ...
   private BigDecimal populatePriceDifference(final AltRoom room, final AltRoom selectedRoom)
   {
      ...
      final List<ExtraFacilityCategory> storeExtraFacilityCategory =
         packageSwapFacade.getExtraFacilityCategory(packageCartService.getBasePackage(), null);
      BigDecimal selectedRoomPrice = BigDecimal.ZERO;
      BigDecimal alternateRoomPrice = BigDecimal.ZERO;
      if (CollectionUtils.isNotEmpty(storeExtraFacilityCategory))
      {
         selectedRoomPrice =
            priceCalculationService.computeRoomCostExcludingExtras(selectedRoom, packageModel
               .getProductType(), storeExtraFacilityCategory, packageModel.getInventory()
               .getInventoryType());
         selectedRoomPrice = selectedRoomPrice.subtract(selectedRoom.getFlightSupplement());
         alternateRoomPrice =
            priceCalculationService.computeRoomCostExcludingExtras(room, packageModel
               .getProductType(), storeExtraFacilityCategory, packageModel.getInventory()
               .getInventoryType());
      ...
      {
         selectedRoomPrice =
            priceCalculationService.computeRoomCostExcludingExtras(selectedRoom,
               packageModel.getProductType(), packageModel.getExtraFacilityCategories(),
   ...
   private List<BoardBasisViewData> populateBoardBasis(final AltRoom room)
   {
      ...
      for (final BoardBasis bb : room.getBoardBasis())
      {
         ...
         // not populating the featureCodeandValues for FC as all the
         // holidays offered are
         // all inclusive and the featureCodeandValues are populated in
         // AllInclusiveBoardFacadeImpl.java
      ...
      final RoomViewData roomViewData, final BigDecimal differencePrice)
   {
      ...
      // Fix DE12810:populating per person price excluding infants as Search
   ...
   private void populateRoomFeatures(final AltRoom room, final RoomViewData roomViewData)
   {
         ...
         .toString(), InventoryType.ATCOM.toString()))
      {
         ...
         roomViewData.setLimitedAvailability(false);
      }
      else
      {
      ...
      final RoomDetails roomDetails)
   {
      ...
      if (SyntacticSugar.isNotNull(roomModel))
      {
         ...
         getRoomImages(roomModel, roomViewData);
    ...
    * @return
    */
   private List<RoomImage> getRoomImages(final RoomModel roomModel, final RoomViewData roomViewData)
   {
      final List<RoomImage> roomImages = new ArrayList<RoomImage>();
      final List<Media> mediaLists = roomMediaService.getImageMedias(roomModel);
      if (CollectionUtils.isNotEmpty(mediaLists))
      {
         for (final Media media : mediaLists)
         {
            if (mediaList.contains(media.getSize().name()))
            {
               final RoomImage roomImage = new RoomImage();
               roomImage.setAltText(media.getAltText());
               roomImage.setUrl(media.getMainSrc());
               roomImage.setCode(media.getCode());
               roomImage.setSize(media.getSize().name());
               roomImage.setDescription(media.getDescription());
               roomImages.add(roomImage);
            }
            else
            {
               roomViewData.setRoomAllocationImage(media.getRoomPlanImages());
            }
         }
      }
    ...
    * (non-Javadoc)
    * 
    * @see uk.co.tui.book.facade.RoomOptionsPageFacade#updateSelectedRoomAllocation
    * (uk.co.tui.book.criteria.RoomSelectionCriteria)
    */
   @Override
      ...
      final RoomViewData roomViewData)
   {
      ...
      for (int i = 0; i < roomViewData.getListOfBoardBasis().size(); i++)
      {
         ...
         totalPerPersonPrice =
            alternateBoardBasis.get(i).getPerPersonPrice()
               .add(roomViewData.getListOfBoardBasis().get(i).getPerPersonPrice());
    ...
    */
   @Override
   public void populateRoomOptionsStaticContentViewData(
      final RoomAndBoardOptionsPageViewData viewData)
   {
    ...
    * (non-Javadoc)
    * 
    * @see uk.co.tui.book.facade.RoomOptionsPageFacade#populateRoomsForWebAnalytics
    * (uk.co.tui.book.view.data. RoomAndBoardOptionsPageViewData)
    */
   @Override
