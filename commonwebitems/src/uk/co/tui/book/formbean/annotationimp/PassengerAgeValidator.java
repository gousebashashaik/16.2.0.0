...
public final class PassengerAgeValidator {
    ...
    public static void validateAge(final PassengerDetailsFormBean formBean,
        ...
        Calendar dobCalendar = Utilities.getDefaultCalendar();

        @SuppressWarnings("deprecation")
        Calendar arrivalDateCalendar = Utilities.getDefaultCalendar();
        Date dateOfBirth = null;
        for (PassengerInfoFormBean passengerInfoFormBean : formBean.getPaxInfoFormBean()) {
            dateOfBirth = getDateOfBirth(passengerInfoFormBean);
            if (SyntacticSugar.isNull(dateOfBirth)
                    || SyntacticSugar.isNull(arrivalDateCalendar)) {
                continue;
            }
    ...
    private static Date getDateOfBirth(final PassengerInfoFormBean passengerInfoFormBean) {
        if(isInfantBorn(passengerInfoFormBean) && childOrInfant(passengerInfoFormBean.getPersonType())) {
            return DateUtils.toUtilDate(
     ...
     */
    private static boolean childOrInfant(String paxType) {
