...
import uk.co.tui.fj.book.populators.ExtraFacilityViewDataPopulator;
import uk.co.tui.fj.book.populators.InsuranceViewDataPopulator;
...
import uk.co.tui.fj.book.view.data.ExtrasToPassengerRelationViewData;
...
public class ExtraFacilityUpdator
{
   ...
   @Resource(name = "fjInsuranceViewDataPopulator")
         ...
         .getExtraFacilityViewData())
      {
         if (bagModel.getExtraFacilityCode().equals(extraFacilityViewData.getCode()))
         {
            ...
            extraFacilityViewData.setCurrencyAppendedPrice(getCurrencyAppendedPrice(
               extraFacilityViewData.getPrice().multiply(
                  BigDecimal.valueOf(extraFacilityViewData.getPassengerExtraFacilityMapping()
                     .getPassengers().size())), currencyResolver.getSiteCurrency()));
            isExtraPresent = true;
      ...
      final ExtraFacilityViewData extraView)
   {
      if (isKidsActivityExtra(extraView) && BigDecimal.ZERO.compareTo(extraView.getPrice()) == 0)
      {
         ...
         extraView.setCurrencyAppendedPrice(getCurrencyAppendedPrice(amount,
            currencyResolver.getSiteCurrency()));
      }
   }
      ...
      final ExtraFacilityViewData extraFacilityViewData)
   {
      if (isKidsActivityExtra(extraFacilityViewData))
      {
         ...
         // TODO need to revisit
         extraFacilityViewData.setCurrencyAppendedPrice(getCurrencyAppendedPrice(amount,
            currencyResolver.getSiteCurrency()));
         // TODO need to revisit, this for displaying the proper quantity in
         // summary panel
   ...
   public void updateSeatExtrasRelativePrice(final FlightOptionsViewData viewData)
   {
      ...
      for (final ExtraFacilityViewData viewExtra : extras)
      {
         ...
         if (StringUtil.equalsIgnoreCase(viewExtra.getCode(), ExtraFacilityConstants.PREMIUM_SEAT))
         {
            ...
            viewExtra.setCurrencyAppendedPrice(getCurrencyAppendedPrice(premiumPrice,
               currencyResolver.getSiteCurrency()));
            viewExtra.setCurrencyAppendedPerPersonPrice(getCurrencyAppendedPrice(
               getCalculatedPrice(
                  price,
                  new BigDecimal(PassengerUtils.getChargeablePaxCount(getBasePackageModel()
                     .getPassengers()))), currencyResolver.getSiteCurrency()));
         }
         else
         {
    ...
    * @param seatOptions
    */
   public void sortSeatsByCode(final ExtraFacilityCategoryViewData seatOptions)
   {
