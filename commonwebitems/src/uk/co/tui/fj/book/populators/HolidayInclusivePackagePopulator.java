...
import static uk.co.tui.fj.book.constants.BookFlowConstants.BRAND_NAME;
import static uk.co.tui.fj.book.constants.BookFlowConstants.CHANNEL;
...
public class HolidayInclusivePackagePopulator implements Populator<Holiday, BasePackage>
{
   ...
   @Resource(name = "fjDepositPopulator")
      ...
      final BasePackage target)
   {
      ...
      {
         if (StringUtil.notEmptyAndNotBlank(productRangeModel.getCode())
            && BrandUtils.isValidBrandCode(productRangeModel.getBrands(),
               getCrossSellingBrand(target)))
         {
    ...
    *
    * @param basePackage the source
    * @return the cross selling brand
    */
   private String getCrossSellingBrand(final BasePackage basePackage)
   {
      if (basePackage.getCrossBrandType() != null)
      {
         return basePackage.getCrossBrandType().getCode();
      }
      return BrandType.FJ.toString();
   }
