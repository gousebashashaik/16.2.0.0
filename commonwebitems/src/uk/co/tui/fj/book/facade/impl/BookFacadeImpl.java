...
import uk.co.tui.fj.book.facade.BookFacade;
import uk.co.tui.fj.book.populators.AlertPassengerViewDataPopulator;
import uk.co.tui.fj.book.populators.AlertPopulator;
import uk.co.tui.fj.book.populators.BookingDetailsViewDataPopulator;
import uk.co.tui.fj.book.populators.HolidayInclusivePackagePopulator;
...
public class BookFacadeImpl implements BookFacade
{
    ...
    @Resource(name = "fjHolidayInclusivePackagePopulator")
    ...
    @Resource(name = "fjAlertPassengerViewDataPopulator")
    private AlertPassengerViewDataPopulator alertPassengerViewDataPopulator;

    @Resource(name = "fjBookingDetailsViewDataPopulator")
     ...
     * (non-Javadoc)
     *
     * @see uk.co.tui.th.book.facade.BookFacade#populateInclusivePackage (uk.co.portaltech.travel.model.results.Holiday)
     */
    @Override
     ...
     */
    public boolean hasPriceChanged(final BasePackage holidayPackage, final BasePackage latestPacakgeFromInventory)
    {
     ...
     */
    @Override
    public List<AlertViewData> checkPriceAndAvailability() throws TUIBusinessException, FlightSoldOutException
    {
        ...
        try
        {
            ...
            feedBackList = checkPriceAvailabilityService.updatePriceAndAvailability(basePackage, InventoryType.ATCOM == basePackage
                    .getInventory().getInventoryType());
        }
        catch (final BookServiceException e)
        {
            LOGGER.error(TUI_EXCEPTION, e);
     ...
     */
    private void handleBookServiceException(final BookServiceException e) throws FlightSoldOutException, TUIBusinessException
    {
        final List<String> multicomErrorCodes = Arrays.asList(StringUtils.split(Config.getParameter("multicom_error_codes"), ','));
        final String errorCode = StringUtils.trim(e.getErrorCode());
        if (multicomErrorCodes.contains(errorCode) || StringUtils.startsWithIgnoreCase(errorCode, MULTICOMERROR_PREFIX))
        {
    ...
    public BookingDetailsViewData confirmBooking()
    {
        ...
        catch (final BookServiceException e)
        {
            ...
            LOGGER.error("Booking Service Exception", e);
        }

        bookingDetailsViewDataPopulator.populate(packageCartService.getBasePackage(), bookingDetailsViewData);
     ...
     * (non-Javadoc)
     *
     * @see uk.co.tui.th.book.facade.BookFacade#getPaymentPageRelativeUrl(uk.co.tui. domain.model.InclusivePackageModel,
     * java.lang.String, java.util.Map)
     */
    @Override
    public String getPaymentPageRelativeUrl(final Map<String, String> urlMap, final String hostName,final String contextPath)
            throws FlightSoldOutException, TUIBusinessException
    {
        ...
        paymentRequest.setPrePaymentUrl(StringUtil.append(hostName, Config.getParameter(brandType + "_PRE_PAYMENT_URL")));
        paymentRequest.setPaymentFailureUrl(StringUtil.append(hostName, Config.getParameter(brandType + "_PAYMENT_FAILURE_URL")));
    ...
    {
        if (packageCartService.getBasePackage().getListOfHighlights().contains(HighLights.MULTI_CENTRE))
        {
            paymentRequest.setMultiCentreData(convertMultiCentreData(linkedItemService.getMultiCentreData(packageComponentService
     ...
     * @return the map
     */
    private Map<String, List<String>> convertMultiCentreData(final List<MultiCentreData> multiCentreData)
    {
        ...
        for (final MultiCentreData eachData : multiCentreData)
        {
            ...
            paymentData.get(duration).add(
                    new StringBuilder(eachData.getDuration()).append(nights).append(eachData.getName()).toString());
        }
        return paymentData;
    }
     ...
     * @throws TUIBusinessException
     */
    private void populateAlertViewData(final PaymentRequest paymentRequest) throws FlightSoldOutException, TUIBusinessException
    {
    ...
    {
        final PackageExtraFacilityStore packageExtraFacilityStore = sessionService.getAttribute("PackageExtraFacilityStore");
