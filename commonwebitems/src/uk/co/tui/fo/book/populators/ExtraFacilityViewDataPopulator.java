   ...
   Populator<ExtraFacility, ExtraFacilityViewData>
{
      ...
      final ExtraFacilityViewData target)
   {
      ...
      final Integer quantity = getQuantity(source);
    ...
    * @return
    */
   private Integer getQuantity(final ExtraFacility source)
   {
      Integer quantity = Integer.valueOf(0);
      for (final Price priceModel : source.getPrices())
      {
         if (SyntacticSugar.isNotNull(priceModel.getQuantity()))
         {
            quantity = quantity + priceModel.getQuantity();
         ...
         totalExtraPrice =
            totalExtraPrice.add(
               source.getPrices().get(1).getRate().getAmount()
                  .multiply(BigDecimal.valueOf(source.getPrices().get(1).getQuantity()))).add(
               source.getPrices().get(0).getRate().getAmount()
                  .multiply(BigDecimal.valueOf(source.getPrices().get(0).getQuantity())));
         target.setCurrencyAppendedPrice(getCurrencyAppendedPrice(
            totalExtraPrice.setScale(TWO, RoundingMode.HALF_UP),
    ...
    * @param source
    */
   private void populatePrice(final ExtraFacilityViewData target, final ExtraFacility source)
   {
      BigDecimal totalPrice = BigDecimal.ZERO;
      totalPrice = addTotalPrice(source, totalPrice);
    ...
    * @return
    */
   private BigDecimal addTotalPrice(final ExtraFacility source, final BigDecimal totalPrice)
   {
      BigDecimal totalPric = totalPrice;
      for (final Price price : source.getPrices())
      {
         final Money moneyModel = price.getAmount();
         if (moneyModel != null && moneyModel.getAmount() != null)
         {
            totalPric = totalPric.add(moneyModel.getAmount());
         }
      }
      return totalPric;
   }
      ...
      final ExtraFacilityViewData target, final boolean description)
   {
      if (BookFlowConstants.TRANSFER_OPTION_CODES.contains(source.getExtraFacilityCode())
         ...
         || source.getExtraFacilityCode().contains(ExtraFacilityConstants.COACH_TRASNFER))
      {
   ...
   private String getDescription(final String key, final String... value)
   {
      if (StringUtils.isNotEmpty(key))
      {
         ...
         summaryContent = getFOSummaryDescription(key, summaryContent);
    ...
    * @return
    */
   private String getFOSummaryDescription(final String key, String summaryContent)
   {
      if (key.contains(ExtraFacilityConstants.COACH_TRASNFER))
      {
         summaryContent = getStaticContent().get(ExtraFacilityConstants.COACH_TRASNFER);
      }
      return summaryContent;
   }
   ...
   private String getTransferDescription(final ExtraFacility source)
   {
      ...
      {
         return getTransferDescriptionNotIncluded(source);
      }
   }
    ...
    * @return
    */
   private String getTransferDescriptionNotIncluded(final ExtraFacility source)
   {
      if (BookFlowConstants.TRANSFER_OPTION_CODES.contains(source.getExtraFacilityCode())
         || source.getExtraFacilityCode().contains(ExtraFacilityConstants.COACH_TRASNFER))
      {
         return getDescription(source.getExtraFacilityCode(), source.getPrices().get(0)
