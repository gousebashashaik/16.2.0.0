...
import uk.co.tui.fo.book.view.data.ExcessWaiverViewData;
 ...
 *
 */
public class InsuranceViewDataPopulatorLite implements Populator<InsuranceExtraFacility, InsuranceViewData>
{
	...
	@Override
	public void populate(final InsuranceExtraFacility extraFacility, final InsuranceViewData insuranceViewData)
			throws ConversionException
	{
		...
		{
			insuranceViewData.setPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode) + frmPrice.toString());
		}
		else
		{
			insuranceViewData.setPaxComposition(getPaxComposition(getTypes(extraFacility.getPassengers())));
		}
		insuranceViewData.setDescriptionSummary(insuranceViewData.getDescription() + insuranceViewData.getPaxComposition());
		insuranceViewData.setCurrencyFrmPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode) + frmPrice.toString());
	 ...
	 *           boolean
	 */
	private void updateCurrencyAppendedPrice(final InsuranceViewData insuranceViewData, final Price price)
	{
		final String defaultCurrencyCode = currencyResolver.getSiteCurrency();
		if (price.getPriceProfile() != null)
		{
			updateCutryAppendedByProfile(insuranceViewData, price, defaultCurrencyCode);
		}
	}

	private void updateCutryAppendedByProfile(final InsuranceViewData insuranceViewData, final Price price,
			final String defaultCurrencyCode)
	{
		...
		else if (StringUtils.equalsIgnoreCase(price.getPriceProfile().getPersonType().toString(), PersonType.CHILD.toString()))
		{
	...
	{
		return StringUtils.equalsIgnoreCase(price.getPriceProfile().getPersonType().toString(), PersonType.ADULT.toString())
				&& StringUtils.isEmpty(insuranceViewData.getFrmAdtPrice());
	}
	 ...
	 * @param insuranceViewData
	 */
	private void updateQuantity(final InsuranceExtraFacility extraFacility, final InsuranceViewData insuranceViewData)
	{
		int insuranceQuantity = 0;
		if (isFamilyPresent(extraFacility) && CollectionUtils.isNotEmpty(extraFacility.getPassengers()))
		{
			insuranceQuantity = 1;
		}
	 ...
	 */
	@SuppressWarnings(BookFlowConstants.BOXING)
	private void populateExWaiverViewData(final InsuranceExtraFacility extraFacility, final InsuranceViewData insuranceViewData)
	{
		if (extraFacility.getInsuranceExcessWaiver() != null)
		{
			final ExcessWaiverViewData excessWaiverViewData = new ExcessWaiverViewData();
			excessWaiverViewData.setCode(extraFacility.getInsuranceExcessWaiver().getExtraFacilityCode());
			excessWaiverViewData.setDescription(extraFacility.getInsuranceExcessWaiver().getDescription());
			excessWaiverViewData.setSelected(BooleanUtils.toBoolean(isExcessWaiverOpted(extraFacility)));
			updateInsurancePrice(extraFacility, insuranceViewData, excessWaiverViewData);
			excessWaiverViewData.setDescriptionSummary(getDescription("EXTRADESCRIPTION", excessWaiverViewData.getDescription(),
	 ...
	 */
	@SuppressWarnings(BookFlowConstants.BOXING)
	private void updateInsurancePrice(final InsuranceExtraFacility extraFacility, final InsuranceViewData insuranceViewData,
			final ExcessWaiverViewData excessWaiverViewData)
	{
		if (extraFacility.getInsuranceExcessWaiver().getPrice() != null)
		{
			...
			if (isNonFamilyExcessWaiverOpted(extraFacility))
			{
				insuranceViewData.setTotalPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
						+ calculateTotalPrice(extraFacility).add(
								extraFacility.getInsuranceExcessWaiver().getPrice().getAmount().getAmount()).toString());
				excessWaiverViewData.setPrice(extraFacility.getInsuranceExcessWaiver().getPrice().getAmount().getAmount().toString());
			}
			else if (isExcessWaiverOpted(extraFacility))
			{
				excessWaiverViewData.setPrice(extraFacility.getInsuranceExcessWaiver().getPrice().getRate().getAmount().toString());
				insuranceViewData.setTotalPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
						+ calculateTotalPrice(extraFacility).add(
								extraFacility.getInsuranceExcessWaiver().getPrice().getRate().getAmount()).toString());
			}
			else
			{
				excessWaiverViewData.setPrice(extraFacility.getInsuranceExcessWaiver().getPrice().getRate().getAmount().toString());
			...
			excessWaiverViewData.setCurrencyAppendedPrice(CurrencyUtils.getCurrencySymbol(currencyResolver.getSiteCurrency())
					+ (new BigDecimal(excessWaiverViewData.getPrice()).setScale(TWO, BigDecimal.ROUND_HALF_UP).toString()));
		}
	}
	...
	private String getPaxComposition(final Collection<String> types)
	{
		...
		getPax(getCount(types, ADULT) + getCount(types, TEEN), ADULT, paxComposition);
	 ...
	 * @param paxComposition
	 */
	private void constructPaxType(final int count, final String type, final StringBuilder paxComposition)
	{
		if (count > 1)
		{
