...
public class PriceBreakDownViewDataPopulator implements Populator<BasePackage, PackageViewData>
{
      ...
      throws ConversionException
   {
      ...
      final PriceBreakDownViewData flightsCost = new PriceBreakDownViewData();
      ...
      final PriceBreakDownViewData apdPrice = new PriceBreakDownViewData();
      final PriceBreakDownViewData taxPrice = new PriceBreakDownViewData();
      ...
      apdPrice.setDescription(getDescription("AIRPASSENGERDUTY"));
      apdPrice.setPrice(priceCalculationService.getAirPassengerDutyPrice(source));
      apdPrice.setCurrencyAppendedPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
         + apdPrice.getPrice().setScale(TWO, RoundingMode.HALF_UP));

      taxPrice.setDescription(getDescription("AIRPORTTAX"));
      taxPrice.setPrice(priceCalculationService.getAirPortTaxPrice(source));
      taxPrice.setCurrencyAppendedPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
         + taxPrice.getPrice().setScale(TWO, RoundingMode.HALF_UP));

      flightsCost.setDescription(getDescription("FLIGHTS"));
      BigDecimal flightprice = priceCalculationService.getFlightsCosts(source, BigDecimal.ZERO);
      flightprice = flightprice.subtract(apdPrice.getPrice());
      flightprice = flightprice.subtract(taxPrice.getPrice());
      flightsCost.setPrice(flightprice.setScale(TWO, RoundingMode.HALF_UP));
      flightsCost.setCurrencyAppendedPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
         + flightsCost.getPrice().setScale(TWO, RoundingMode.HALF_UP).toString());
      target.add(flightsCost);

      // Moving piece of code for ordering in summary panel.
      if (apdPrice.getPrice().compareTo(BigDecimal.ZERO) > 0)
      {
         target.add(apdPrice);
      }

      if (taxPrice.getPrice().compareTo(BigDecimal.ZERO) > 0)
      {
         target.add(taxPrice);
      }
      ...
      final List<PriceBreakDownViewData> target)
   {
            ...
            .getPaymentReferences()))
      {
         ...
         cardCharges.setCurrencyAppendedPrice(CurrencyUtils.getCurrencySymbol(currencyResolver
            .getSiteCurrency()) + cardCharge.setScale(TWO, RoundingMode.HALF_UP));
