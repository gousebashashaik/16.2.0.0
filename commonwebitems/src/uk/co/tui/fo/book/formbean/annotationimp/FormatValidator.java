...
/**
 * @author pradeep.as Implementation class to check Not Null and regex condition.
 */
public class FormatValidator implements ConstraintValidator<ValidateFormat, String>
{
    ...
    */
   @Override
   public void initialize(final ValidateFormat format)
   {
      regEx = format.regEx();
   ...
   public boolean isValid(final String field, final ConstraintValidatorContext context)
   {
      ...
      else if (invalidContent(field))
      {
         context.disableDefaultConstraintViolation();
         context.buildConstraintViolationWithTemplate(
            createMessageCode(PassengerDetailConstants.PATTERN_CODE, fieldmessCode))
            .addConstraintViolation();
         result = false;
   ...
   private boolean invalidContent(final String field)
   {
      return (StringUtils.isNotBlank(field))
         && (isFieldExceedsMaxValueOrMinValue(field) || isFieldNotMatchesRegEx(field));
   }
   ...
   {
      return StringUtils.isNotBlank(regEx) && !field.trim().matches(regEx);
   }
    ...
    */
   private boolean isFieldExceedsMaxValueOrMinValue(final String field)
   {
      return field.trim().length() > max || field.trim().length() < min;
   }
    ...
    * @return the string
    */
   public String createMessageCode(final String errorCode, final String fieldCode)
   {
