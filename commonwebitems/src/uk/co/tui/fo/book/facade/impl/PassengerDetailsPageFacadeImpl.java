...
import uk.co.tui.domain.model.ISOCountryCodesModel;
import uk.co.tui.fo.book.ExtraFacilityUpdator;
import uk.co.tui.fo.book.constants.BookFlowConstants;
import uk.co.tui.fo.book.facade.PassengerDetailsPageFacade;
import uk.co.tui.fo.book.populators.AlertViewDataPopulator;
...
import uk.co.tui.fo.book.view.data.DataProtectionViewData;
import uk.co.tui.fo.book.view.data.InfantNotYetBornViewData;
import uk.co.tui.fo.book.view.data.MemoViewData;
...
import uk.co.tui.fo.book.view.data.PassengerInsuranceViewData;
import uk.co.tui.fo.book.view.data.PromotionalCodeViewData;
...
public class PassengerDetailsPageFacadeImpl implements PassengerDetailsPageFacade
{
	...
	private static final int CHILD_AGE_LIMIT = 15;
	...
	@Resource(name = "foPackageViewDataPopulator")
	...
	@Resource(name = "foDataProtectionViewDataPopulator")
	...
	public PackageViewData renderPackageViewData(final PassengerDetailsViewData paxViewData)
	{
		...
		paxViewData.setCountryCodes(getCountryCodes());
	...
	private Map<String, String> getCountryCodes()
	{
		...
		final List<ISOCountryCodesModel> isoCountryCodeModels = iSOCountryCodeService.findCountries();
		for (final ISOCountryCodesModel isoCountryCodeModel : isoCountryCodeModels)
		{
			countryCodes.put(isoCountryCodeModel.getAlpha2Code(), isoCountryCodeModel.getShortName());
		}
		return sortByValues(countryCodes);
	}

	private static <K extends Comparable, V extends Comparable> Map<K, V> sortByValues(final Map<K, V> map)
	{
	...
	private void familyInsNotPresent(final BasePackage packageModel, final PassengerInsuranceViewData passengerInsuranceViewData)
	{
		...
		final int teenCount = getCount(passengersList, PersonType.TEEN);
		...
		final int teenInsuranceCount = getInsuranceSelectedCount(passengersList, PersonType.TEEN);
		...
		updtTeenPassData(teenCount, teenInsuranceCount, passengerDataList, leadPassenger);
	 ...
	 * @param leadPassenger
	 */
	private void updtTeenPassData(final int teenCount, final int teenInsuranceCount, final List<PassengerData> passengerDataList,
			final PersonType leadPassenger)
	{
		if (teenCount != ZERO && teenInsuranceCount != ZERO)
		{
			passengerDataList.add(setPassengerData(teenCount, teenInsuranceCount, PersonType.TEEN, leadPassenger));
		}
	}
	...
	private List<Feedback> performInfoBook(final BasePackage pkg, final PassengerDetailsViewData paxViewdata)
	{
		...
		catch (final BookServiceException e)
		{
			...
			LOG.error("TUISystemException : " + e.getMessage(), e);
			return feedBackList;
		}
		return feedBackList;
	}
	...
	public List<Feedback> performInfoBook(final BasePackage pkg) throws TUIBusinessException, FlightSoldOutException
	{
		...
		catch (final BookServiceException e)
		{
			...
			{
				throw new uk.co.tui.fo.exception.TUIBusinessException(e.getErrorCode(), e.getCustomMessage(), e);
			}
			else
			{
	...
	public CommunicationPreferencesViewData getCommunicationPreferences()
	{
		...
		{
			final uk.co.tui.book.domain.lite.CommunicationPreference comPreferenceModel = packageModel.getCommunicationPreference();
