...
import uk.co.tui.fo.book.facade.FlightExtraFacilityFacade;
...
public class FlightExtraFacilityFacadeImpl implements FlightExtraFacilityFacade
{
      ...
      final Map<String, String> baggageAllocationMap)
   {
      ...
      {
         removeDefaultBaggageFromPackageModel(packageModel);
      }
      ...
      updateViewDataWithTotalSelected(PassengerUtils.getPersonTypeCountFromPassengers(
         ...
         EnumSet.of(PersonType.ADULT, PersonType.TEEN, PersonType.CHILD)), viewData
   ...
   private void updateDefaultBaggageForSelectedSeat(final BasePackage packageModel,
      final Map<String, List<ExtraFacility>> validExtraFacilitiesMap)
   {
      removeBaggageExtraIfPresent(packageModel);
      final Map<List<ExtraFacility>, List<String>> extrasToPassengersMap =
         new HashMap<List<ExtraFacility>, List<String>>();
      for (final List<ExtraFacility> extras : validExtraFacilitiesMap.values())
      {
         ...
         {
            final Collection<Passenger> applicablePasengers =
               PassengerUtils.getApplicablePassengers(packageModel.getPassengers(), EnumSet.of(
    ...
    * @param packageModel
    */
   private void removeBaggageExtraIfPresent(final BasePackage packageModel)
   {
      final Iterator<ExtraFacilityCategory> iterator =
         packageModel.getExtraFacilityCategories().iterator();
      while (iterator.hasNext())
      {
         final ExtraFacilityCategory extraCategory = iterator.next();
         if (StringUtils.equalsIgnoreCase("BAG", extraCategory.getCode()))
         {
            iterator.remove();
         }
      }
   ...
   public void updateBaggageForPremiumSeat(final BasePackage packageModel,
      final String selectedSeatCabinClass)
   {
      addDefaultBaggageExtrasToPackageModel(packageModel,
         getDefaultPremiumBaggageExtrasLite(packageModel, selectedSeatCabinClass));
    ...
    * @param packageModel
    */
   private void removeDefaultBaggageFromPackageModel(final BasePackage packageModel)
   {
      final Iterator<ExtraFacilityCategory> iterator =
         packageModel.getExtraFacilityCategories().iterator();
      while (iterator.hasNext())
      {
         if (StringUtils.equalsIgnoreCase(iterator.next().getCode(),
    ...
    */
   @SuppressWarnings(SUPRESS_WARNING_BOXING)
   private List<ExtraFacility> getDefaultPremiumBaggageExtrasLite(final BasePackage packageModel,
      final String selectedSeatCabinClass)
   {
      ...
      final FlightExtraFacilityStore flightExtraStore =
         (FlightExtraFacilityStore) sessionService
            .getAttribute(SessionObjectKeys.FLIGHT_EXTRA_FACILITY_STORE);
      for (final List<ExtraFacility> extras : flightExtraStore
         .getExtraFacilityFromAllLegsBasedOnCabinClass(packageModel.getId(), selectedSeatCabinClass)
         .values())
      {
         if (StringUtils.equalsIgnoreCase(extras.get(0).getExtraFacilityCategory().getCode(),
            ExtraFacilityConstants.BAGGAGE_EXTRA_CATEGORY_CODE))
      ...
      for (final ExtraFacilityCategory categoryModel : packageModel.getExtraFacilityCategories())
      {
            ...
            ExtraFacilityConstants.BAGGAGE_EXTRA_CATEGORY_CODE))
         {
            for (final ExtraFacility extra : categoryModel.getExtraFacilities())
            {
               ...
               bagExtra.setSelection(FacilitySelectionType.INCLUDED);
               bagExtra.setSelected(true);
   ...
   {
      return Boolean
         .valueOf(BooleanUtils.toBoolean(Boolean.valueOf(extra.isSelected())) && CollectionUtils
            .isNotEmpty(extra.getPassengers()));
   }
