...
import uk.co.tui.th.book.view.data.ExcessWaiverViewData;
   ...
   Populator<InsuranceExtraFacility, InsuranceViewData>
{
      ...
      final InsuranceViewData insuranceViewData) throws ConversionException
   {
      ...
      insuranceViewData.setFamilyInsPresent(isFamilyInsurance(extraFacility));
      ...
      {
         if (insuranceViewData.isRenderNewInsurance())
         {
            updateAllCurrencyAppendedPrice(insuranceViewData, price);
      ...
      insuranceViewData.setRoundedTotalPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
         + (calculateTotalPrice(extraFacility)).setScale(0, RoundingMode.HALF_UP).toString());
      insuranceViewData.setPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
         + calculateTotalPrice(extraFacility).toString());
      insuranceViewData.setRoundedPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
         + (calculateTotalPrice(extraFacility)).setScale(0, RoundingMode.HALF_UP).toString());
      insuranceViewData.setFrmPrice(frmPrice.toString());
      insuranceViewData.setRoundedFrmPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
         + frmPrice.setScale(0, RoundingMode.HALF_UP).toString());
      ...
      populateExcessWaiverData(extraFacility, insuranceViewData, defaultCurrencyCode);
   }
    ...
    */
   private void populateExcessWaiverData(final InsuranceExtraFacility extraFacility,
      final InsuranceViewData insuranceViewData, final String defaultCurrencyCode)
   {
      if (extraFacility.getInsuranceExcessWaiver() != null)
      {
         BigDecimal insWithExcessWaiverPrice = BigDecimal.ZERO;
         insWithExcessWaiverPrice = populateInsWithExcesswaiver(extraFacility, insuranceViewData);
         populateInsWithExcessWaiver(insuranceViewData, defaultCurrencyCode,
            insWithExcessWaiverPrice);
      }
   }
   
   private boolean isFamilyInsurance(final InsuranceExtraFacility extraFacility)
   {
    ...
    * @return
    */
   private BigDecimal populateInsWithExcesswaiver(final InsuranceExtraFacility extraFacility,
      final InsuranceViewData insuranceViewData)
   {
      BigDecimal insWithExcessWaiverPrice;
      if (isFamilyInsurance(extraFacility))
      {
         insWithExcessWaiverPrice =
            new BigDecimal(insuranceViewData.getFrmPrice()).add(new BigDecimal(insuranceViewData
               .getExcessWaiverViewData().getPrice()));
      }
      else
      {
         insWithExcessWaiverPrice =
            calculateTotalPrice(extraFacility).add(
               new BigDecimal(insuranceViewData.getExcessWaiverViewData().getPrice()));
      }
      return insWithExcessWaiverPrice;
   }
   
   /**
    * @param insuranceViewData
    * @param defaultCurrencyCode
    * @param insWithExcessWaiverPrice
    */
   private void populateInsWithExcessWaiver(final InsuranceViewData insuranceViewData,
      final String defaultCurrencyCode, final BigDecimal insWithExcessWaiverPrice)
   {
      if (insuranceViewData.isRenderNewInsurance())
      {
         insuranceViewData.setCurrencyAppendedInsWithExcessWaiver(CurrencyUtils
            .getCurrencySymbol(defaultCurrencyCode)
            + (insWithExcessWaiverPrice.setScale(0, RoundingMode.HALF_UP)).toString());
      }
   }
    ...
    * @param price boolean
    */
   private void updateAllCurrencyAppendedPrice(final InsuranceViewData insuranceViewData,
      final Price price)
   {
      final String defaultCurrencyCode = currencyResolver.getSiteCurrency();
      if (price.getPriceProfile() != null)
      {
         ...
         {
            final BigDecimal roundedPrice = roundingOffPrice(price);
            insuranceViewData.setFrmAdtPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
               + roundedPrice.toString());
         }
         getInusrnacePriceForSeniorsAndChild(insuranceViewData, price, defaultCurrencyCode);
      }
   }
    ...
    * @param defaultCurrencyCode
    */
   private void getInusrnacePriceForSeniorsAndChild(final InsuranceViewData insuranceViewData,
      final Price price, final String defaultCurrencyCode)
   {
      if (isSeniorPriceUpdate(insuranceViewData, price))
      {
         final BigDecimal roundedPrice = roundingOffPrice(price);
         insuranceViewData.setFrmSeniorPrice(CurrencyUtils.getCurrencySymbol(defaultCurrencyCode)
            + roundedPrice.toString());
      }
      if (isSuperSeniorPriceUpdate(insuranceViewData, price))
      {
         final BigDecimal roundedPrice = roundingOffPrice(price);
         insuranceViewData.setFrmSuperSeniorPrice(CurrencyUtils
            .getCurrencySymbol(defaultCurrencyCode) + roundedPrice.toString());
      }
    ...
    * @return
    */
   private BigDecimal roundingOffPrice(final Price price)
   {
      return price.getRate().getAmount().setScale(0, RoundingMode.HALF_UP);
      
   }
   
   /**
    * Checks if senior price update required
    *
    * @param insuranceViewData
    * @param price
    * @return boolean
    */
   private boolean isSeniorPriceUpdate(final InsuranceViewData insuranceViewData, final Price price)
   {
      return StringUtils.equalsIgnoreCase(price.getPriceProfile().getPersonType().toString(),
         PersonType.SENIOR.toString())
         && StringUtils.isEmpty(insuranceViewData.getFrmSeniorPrice());
   }
    ...
    * @return boolean
    */
   private boolean isSuperSeniorPriceUpdate(final InsuranceViewData insuranceViewData,
      final Price price)
   {
      return StringUtils.equalsIgnoreCase(price.getPriceProfile().getPersonType().toString(),
         PersonType.SUPERSENIOR.toString())
         && StringUtils.isEmpty(insuranceViewData.getFrmSuperSeniorPrice());
   }
      ...
      final InsuranceViewData insuranceViewData, final ExcessWaiverViewData excessWaiverViewData)
   {
      if (extraFacility.getInsuranceExcessWaiver().getPrice() != null)
      {
         ...
         if (isNonFamilyExcessWaiverOpted(extraFacility))
         {
            ...
            excessWaiverViewData.setExcessWaiverprice(extraFacility.getInsuranceExcessWaiver()
               .getPrice().getRate().getAmount().toString());
         }
   ...
   {
      return isExcessWaiverOpted(extraFacility) && !isFamilyInsurance(extraFacility)
