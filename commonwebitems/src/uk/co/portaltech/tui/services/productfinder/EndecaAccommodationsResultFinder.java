 ...
 *
 */
public class EndecaAccommodationsResultFinder extends AbstractEndecaFinder implements ProductFinder
{

   private static final TUILogUtils LOG = new TUILogUtils("EndecaAccommodationsResultFinder");

   @Override
   // @Cacheable(cacheName = "endecaBrowseCache")
   public SearchResultData<ResultData> search(final SearchRequestData request)
   {
      LOG.debug("Searching Filtered Accommodation Results.");
      final AccommodationsResultComponentModel component =
         (AccommodationsResultComponentModel) request.getRelevantItem();
