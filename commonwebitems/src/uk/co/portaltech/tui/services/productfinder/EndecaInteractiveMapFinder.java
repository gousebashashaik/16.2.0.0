...
public final class EndecaInteractiveMapFinder extends AbstractEndecaFinder implements ProductFinder
{

   private static final TUILogUtils LOG = new TUILogUtils("EndecaInteractiveMapFinder");

   @Override
   // @Cacheable(cacheName = "endecaBrowseCache")
   public SearchResultData<ResultData> search(final SearchRequestData request)
   {
      LOG.debug("Searching endeca Interactive map result.");
      final InteractiveMapComponentModel carouselModel =
         (InteractiveMapComponentModel) request.getRelevantItem();
