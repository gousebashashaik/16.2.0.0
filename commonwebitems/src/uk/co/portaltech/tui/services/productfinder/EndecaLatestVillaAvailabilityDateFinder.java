 ...
 *
 */
public class EndecaLatestVillaAvailabilityDateFinder extends AbstractEndecaFinder implements ProductFinder
{



    private static final TUILogUtils LOG = new TUILogUtils("EndecaLatestVillaAvailabilityDateFinder");

    @Override
    public SearchResultData<ResultData> search(final SearchRequestData request)
    {
        LOG.debug("Searching for Latest Villa Availablity.");
        final LatestVillaAvailabilityDateComponentModel villaAvailabilityModel = (LatestVillaAvailabilityDateComponentModel) request
                .getRelevantItem();
        return (SearchResultData) processRequest(request, villaAvailabilityModel.getLookupType().toString());
    }
