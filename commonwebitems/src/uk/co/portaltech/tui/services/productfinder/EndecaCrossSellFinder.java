...
public final class EndecaCrossSellFinder extends AbstractEndecaFinder implements ProductFinder
{

   private static final TUILogUtils LOG = new TUILogUtils("EndecaCrossSellFinder");

   @Override
   // @Cacheable(cacheName = "endecaBrowseCache")
   public SearchResultData<ResultData> search(final SearchRequestData request)
   {
      LOG.debug("Searching for cross sell .");
      final CrossSellCarouselComponentModel carouselModel =
         (CrossSellCarouselComponentModel) request.getRelevantItem();
