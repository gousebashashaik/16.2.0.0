...
public final class EndecaUpsellHolidayFinder extends AbstractEndecaFinder implements ProductFinder
{


    private static final TUILogUtils LOG = new TUILogUtils("EndecaUpsellHolidayFinder");

    @Override
    @Cacheable(cacheName = "endecaBrowseCache")
    public SearchResultData<ResultData> search(final SearchRequestData request)
    {
        LOG.debug("Searching for up sell .");
        final UpsellToHolidayComponentModel carouselModel = (UpsellToHolidayComponentModel) request.getRelevantItem();
